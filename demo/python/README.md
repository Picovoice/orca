...
Replace `${ACCESS_KEY}` with your `AccessKey` obtained from Picovoice Console, `${MODEL_PATH}` with a path to any of the model files available under [lib/common](../../lib/common), and `${TEXT}` with your text to be streamed to Orca. Please note that this demo was not tested on macOS.
...
Replace `${ACCESS_KEY}` with yours obtained from Picovoice Console, `${MODEL_PATH}` with a path to any of the model files available under [lib/common](../../lib/common), `${TEXT}` with your text to be synthesized,
