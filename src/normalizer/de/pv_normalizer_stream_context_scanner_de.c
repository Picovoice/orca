...
static bool pv_normalizer_stream_context_can_be_ordinal_dot(const pv_normalizer_token_t *token);

static bool pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(const pv_normalizer_token_t *token);

bool PV_MOCKABLE(pv_normalizer_stream_context_scanner_de_is_verbalizable)(
    ...
    if (token->tag == PV_NORMALIZER_TAG_DE_SPECIAL_CHARACTER) {
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (use_cases[j] == PV_NORMALIZER_USE_DIGITS_SEQUENCE_NORMALIZER_DE) {
                if (pv_normalizer_stream_context_can_be_phone_number(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_FRACTION_NORMALIZER_DE) {
    ...
    if (token->tag == PV_NORMALIZER_TAG_DE_CURRENCY_SYMBOL) {
        bool is_space_currency = (token->previous != NULL && token->previous->tag == PV_NORMALIZER_TAG_DE_SPACE &&
            token->previous->previous !=NULL && (token->previous->previous->tag == PV_NORMALIZER_TAG_DE_CARDINAL ||
                token->previous->previous->tag == PV_NORMALIZER_TAG_DE_NEGATIVE_CARDINAL));

        bool is_non_space_currency = (token->previous != NULL &&
            (token->previous->tag == PV_NORMALIZER_TAG_DE_CARDINAL ||
                token->previous->tag == PV_NORMALIZER_TAG_DE_NEGATIVE_CARDINAL ||
                token->previous->tag == PV_NORMALIZER_TAG_DE_DECIMAL_DIGITS));
    ...
    if (token->tag == PV_NORMALIZER_TAG_DE_SPACE) {
        is_verbalizable = true;
    }

    if ((token->previous != NULL) && (token->previous->tag == PV_NORMALIZER_TAG_DE_MEASUREMENT)) {
        is_verbalizable = false;
    }
    ...
    if (is_after_dot) {
        if (token->tag == PV_NORMALIZER_TAG_DE_SPACE) {
            ...
            if (pv_normalizer_stream_context_can_be_ordinal_dot(token->previous)) {
                is_verbalizable = false;
            }

            if (pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(token->previous)) {
                is_verbalizable = false;
            }
        }
    }
    ...
    if (token->tag == PV_NORMALIZER_TAG_DE_PUNCTUATION) {
        ...
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (
                    (use_cases[j] == PV_NORMALIZER_USE_URL_NORMALIZER_DE) ||
                    use_cases[j] == PV_NORMALIZER_USE_ABBREVIATION_NORMALIZER_DE) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_TIME_NORMALIZER_DE) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_time(token)) {
                    return false;
                }
            } else if (
                    (use_cases[j] == PV_NORMALIZER_USE_CARDINAL_NORMALIZER_DE) ||
                    use_cases[j] == PV_NORMALIZER_USE_NEGATIVE_CARDINAL_NORMALIZER_DE) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_cardinal(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_DECIMAL_NORMALIZER_DE) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_decimal(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_ORDINAL_NORMALIZER_DE) {
                if (pv_normalizer_stream_context_can_be_ordinal_dot(token)) {
                    return false;
                }
            }
        }
...
void PV_MOCKABLE(pv_normalizer_stream_context_scanner_de_remove_hyphen_only_tokens)(
    ...
    while (current) {
        ...
        if (current->tag != PV_NORMALIZER_TAG_DE_NUMBER_RANGE_TO) {
...
pv_status_t pv_normalizer_stream_context_can_be_time(const pv_normalizer_token_t *token) {
    ...
    return (((token->tag == PV_NORMALIZER_TAG_DE_CARDINAL) &&
            ...
            ((token->tag == PV_NORMALIZER_TAG_DE_ALPHANUM_SPELL_OUT) && (strlen(token->string) <= 3)));
}
...
bool pv_normalizer_stream_context_can_be_ordinal_dot(const pv_normalizer_token_t *token) {
    PV_ASSERT(token);
...
bool pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(const pv_normalizer_token_t *token) {
    ...
    bool previous_is_punctuation = (token->previous != NULL) && (token->previous->tag == PV_NORMALIZER_TAG_DE_PUNCTUATION);
