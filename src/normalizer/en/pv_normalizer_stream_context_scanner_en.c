...
bool PV_MOCKABLE(pv_normalizer_stream_context_scanner_en_is_verbalizable)(
    ...
    if (token->tag == PV_NORMALIZER_TAG_EN_SPECIAL_CHARACTER) {
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (use_cases[j] == PV_NORMALIZER_USE_DIGITS_SEQUENCE_NORMALIZER) {
                if (pv_normalizer_stream_context_can_be_phone_number(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_FRACTION_NORMALIZER) {
    ...
    if (token->tag == PV_NORMALIZER_TAG_EN_SPACE) {
        if (token->previous != NULL &&
            (token->previous->tag == PV_NORMALIZER_TAG_EN_DATE_MONTH ||
                token->previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL ||
                token->previous->tag == PV_NORMALIZER_TAG_EN_MEASUREMENT ||
                token->previous->tag == PV_NORMALIZER_TAG_EN_DATE_DAY)) {
            return false;
        }
        return true;
    }
    ...
    if (token->tag == PV_NORMALIZER_TAG_EN_PUNCTUATION) {
        ...
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (
                    (use_cases[j] == PV_NORMALIZER_USE_URL_NORMALIZER) ||
                    use_cases[j] == PV_NORMALIZER_USE_ABBREVIATION_NORMALIZER) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_TIME_NORMALIZER) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_time(token)) {
                    return false;
                }
            } else if (
                    (use_cases[j] == PV_NORMALIZER_USE_CARDINAL_NORMALIZER) ||
                    use_cases[j] == PV_NORMALIZER_USE_NEGATIVE_CARDINAL_NORMALIZER) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_cardinal(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_DECIMAL_NORMALIZER) {
...
void PV_MOCKABLE(pv_normalizer_stream_context_scanner_en_remove_hyphen_only_tokens)(
    ...
    while (current) {
        ...
        if (current->tag != PV_NORMALIZER_TAG_EN_NUMBER_RANGE_TO) {
...
pv_status_t pv_normalizer_stream_context_can_be_time(const pv_normalizer_token_t *token) {
    ...
    return (((token->tag == PV_NORMALIZER_TAG_EN_CARDINAL) &&
            ...
            ((token->tag == PV_NORMALIZER_TAG_EN_ALPHANUM_SPELL_OUT) && (strlen(token->string) <= 3)));
}
...
bool pv_normalizer_stream_context_is_punctuation_part_of_cardinal(const pv_normalizer_token_t *token) {
    ...
    return (strcmp(token->string, ",") == 0) &&
           ((token->previous != NULL) &&
            ((token->previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL) ||
             (token->previous->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL)));
}
...
bool pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(const pv_normalizer_token_t *token) {
    ...
    bool previous_is_punctuation = (token->previous != NULL) && (token->previous->tag == PV_NORMALIZER_TAG_EN_PUNCTUATION);
