...
struct pv_normalizer_tagger_en {
...
static pv_status_t pv_normalizer_tagger_en_tag_postprocess(pv_normalizer_token_list_t *token_list);
...
static pv_status_t pv_normalizer_tagger_en_merge_word_token(pv_normalizer_token_list_t *token_list);
...
pv_status_t PV_MOCKABLE(pv_normalizer_tagger_en_init)(
    ...
    *object = NULL;
    ...
    pv_normalizer_tagger_en_t *o = calloc(1, sizeof(pv_normalizer_tagger_en_t));
    if (!o) {
        PV_ERROR_REPORT(
                &pv_error_msg_alloc,
    ...
    pv_status_t status = pv_normalizer_util_infer_language_from_language_info(o->language_info, &(o->language));
    ...
    for (int32_t i = 0; i < num_use_cases; i++) {
        if (use_cases[i] == PV_NORMALIZER_USE_MEASUREMENT_NORMALIZER) {
            ...
            status = pv_normalizer_util_trie_create(
                    num_normalizable_characters,
                    normalizable_characters,
                    PV_ARRAY_LEN(PV_NORMALIZER_MEASUREMENTS),
                    (const char **) PV_NORMALIZER_MEASUREMENTS,
                    &(o->measurement_trie));
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
...
static pv_status_t pv_normalizer_tagger_en_tag_token(
        const pv_normalizer_tagger_en_t *object,
        pv_normalizer_token_t *token,
        pv_normalizer_token_list_t *token_list) {
    PV_ASSERT(object);
    ...
    for (int32_t j = 0; j < object->num_use_cases; j++) {
        ...
        if (object->use_cases[j] == PV_NORMALIZER_USE_WORD_NORMALIZER) {
            ...
            pv_status_t status = pv_normalizer_tagger_en_tag_time(token, token_list);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                        pv_normalizer_tagger_en_tag_time,
                        pv_status_to_string(status));
                return status;
            }
...
pv_status_t PV_MOCKABLE(pv_normalizer_tagger_en_tag)(
    ...
    while (token != NULL) {
        ...
        pv_status_t status = pv_normalizer_tagger_en_tag_token(object, token, token_list);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                    pv_normalizer_tagger_en_tag_token,
                    pv_status_to_string(status));
            return status;
        }

        bool did_split = false;
        if (split_untagged && (token->tag == PV_NORMALIZER_TAG_EN_NONE)) {
            pv_normalizer_token_list_t *split_token_list = NULL;

            status = pv_normalizer_tagger_en_split_untagged_token(
                    object,
                    &token,
                    &split_token_list,
                    &did_split);
    ...
    pv_status_t status = pv_normalizer_tagger_en_tag_postprocess(token_list);
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_normalizer_tagger_en_tag_postprocess,
                pv_status_to_string(status));
        return status;
    }
...
pv_status_t pv_normalizer_tagger_en_tag_postprocess(pv_normalizer_token_list_t *token_list) {
    PV_ASSERT(token_list);

    pv_status_t status = pv_normalizer_tagger_en_merge_word_token(token_list);
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_normalizer_tagger_en_merge_word_token,
                pv_status_to_string(status));
        return status;
    }
    ...
    while (token != NULL) {
        bool is_spell_out =
                (token->tag == PV_NORMALIZER_TAG_EN_ALPHANUM_SPELL_OUT) ||
                (token->tag == PV_NORMALIZER_TAG_EN_LETTER_SPELL_OUT) ||
                (token->tag == PV_NORMALIZER_TAG_EN_ACRONYM) ||
                (token->tag == PV_NORMALIZER_TAG_EN_NAME_INITIAL_LETTER) ||
                ((token->tag == PV_NORMALIZER_TAG_EN_TOP_LEVEL_DOMAIN) &&
                 PV_NORMALIZER_TOP_LEVEL_DOMAINS_IS_SPELL_OUT[token->tag_data_index]);
        if (is_spell_out) {
            ...
            while (strlen(current->string) >= 2) {
                ...
                if (isdigit(current->string[0])) {
                    current->tag = PV_NORMALIZER_TAG_EN_CARDINAL_SPELL_OUT;
                } else {
                    current->tag = PV_NORMALIZER_TAG_EN_LETTER_SPELL_OUT;
                }
...
pv_status_t pv_normalizer_tagger_tag_word(const pv_language_info_t *language_info, pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_WORD]) {
        pv_status_t status = pv_normalizer_tagger_tag_word_common(language_info, token, PV_NORMALIZER_TAG_EN_WORD);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
...
pv_status_t pv_normalizer_tagger_tag_punctuation(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_PUNCTUATION]) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_SINGLE_QUOTE]) {
        if (strcmp(token->string, APOSTROPHE) == 0) {
            token->tag = PV_NORMALIZER_TAG_EN_SINGLE_QUOTE;
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_cardinal(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_CARDINAL]) {
        const char *string = token->string;
        if (pv_normalizer_util_only_contains_digits(string)) {
            token->tag = PV_NORMALIZER_TAG_EN_CARDINAL;
        } else {
            return PV_STATUS_SUCCESS;
        }
...
void pv_normalizer_tagger_tag_negative_cardinal(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL]) {
        const char *string = token->string;
        ...
        if (pv_normalizer_util_only_contains_digits(string)) {
            token->tag = PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL;
        }
    }
}
...
void pv_normalizer_tagger_tag_custom_pronunciation(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_CUSTOM_PRONUNCIATION]) {
...
pv_status_t PV_MOCKABLE(pv_normalizer_tagger_en_split_untagged_token)(
    ...
    *did_split = false;
    ...
    if ((*token)->tag == PV_NORMALIZER_TAG_EN_NONE) {
        const char *string = (*token)->string;
...
pv_status_t pv_normalizer_tagger_tag_number_range(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_NUMBER_RANGE_TO]) {
        const char *string = token->string;
        ...
        if (is_range && (num_hyphens == 1) && (num_cardinals == 2)) {
            ...
            token->next->tag = PV_NORMALIZER_TAG_EN_NUMBER_RANGE_TO;
        }
    }
    return PV_STATUS_SUCCESS;
}

static bool is_valid_st_ordinal(const char *string, int32_t length) {
    PV_ASSERT(string);
    PV_ASSERT(length > 2);

    if ((string[length - 2] == 'S') &&
        (string[length - 1 == 'T']) &&
        !((length > 3) &&
          (string[length - 4] == '1')) &&
        (string[length - 3] == '1')) {
...
static bool is_valid_nd_ordinal(const char *string, int32_t length) {
    PV_ASSERT(string);
    PV_ASSERT(length > 2);
...
static bool is_valid_rd_ordinal(const char *string, int32_t length) {
    PV_ASSERT(string);
    PV_ASSERT(length > 2);
...
static bool is_valid_th_ordinal(const char *string, int32_t length) {
    PV_ASSERT(string);
    PV_ASSERT(length > 2);
...
pv_status_t pv_normalizer_tagger_tag_ordinal(pv_normalizer_token_t *token, pv_normalizer_token_list_t *token_list) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_ORDINAL]) {
        ...
        if (length >= 3) {
            ...
            if (!found_non_digit) {
                bool is_valid_ending =
                        is_valid_st_ordinal(string, length) ||
                        is_valid_nd_ordinal(string, length) ||
                        is_valid_rd_ordinal(string, length) ||
                        is_valid_th_ordinal(string, length);
                ...
                if (is_valid_ending) {
                    ...
                    if (!pv_normalizer_util_string_number_greater_than_int(number_string, MAX_VERBALIZED_CARDINAL)) {
                        token->tag = PV_NORMALIZER_TAG_EN_ORDINAL;
                    }
                    free(number_string);

                    // Possible final 3 digits of comma number eg. `1,333rd`
                    bool is_before_apostrophe = token->next != NULL && strcmp(token->next->string, APOSTROPHE) == 0;
...
pv_status_t pv_normalizer_tagger_tag_negative_ordinal(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_NEGATIVE_ORDINAL]) {
        char *string = NULL;
        pv_status_t status = pv_normalizer_util_upper(token->string, &string);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
        ...
        if (is_valid_ending) {
            ...
            if (!pv_normalizer_util_string_number_greater_than_int(number_string, MAX_VERBALIZED_CARDINAL)) {
                token->tag = PV_NORMALIZER_TAG_EN_NEGATIVE_ORDINAL;
            }
            free(number_string);
        }
...
pv_status_t pv_normalizer_tagger_tag_special_character(const pv_normalizer_tagger_en_t *object, pv_normalizer_token_t *token) {
    PV_ASSERT(token);

    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_SPECIAL_CHARACTER]) {
        const char *string = token->string;
        ...
        if (is_special_character && special_character_length == (int32_t) strlen(string)) {
            token->tag = PV_NORMALIZER_TAG_EN_SPECIAL_CHARACTER;
        }
    }
    return PV_STATUS_SUCCESS;
}
...
void pv_normalizer_tagger_tag_decimal(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DECIMAL_DIGITS]) {
        ...
        if (has_dot_before_current) {
            ...
            bool has_space_or_null_before_dot =
                    ...
                    (two_previous->tag == PV_NORMALIZER_TAG_EN_SPACE);
            bool has_cardinal_before_dot =
                    ((two_previous != NULL) &&
                     ((two_previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL) ||
                      (two_previous->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL)) &&
                     !two_previous->next_character_is_space);
            ...
            if (has_space_or_null_before_dot || has_cardinal_before_dot) {
                ...
                if (!found_non_digit) {
                    token->tag = PV_NORMALIZER_TAG_EN_DECIMAL_DIGITS;
                    token->previous->tag = PV_NORMALIZER_TAG_EN_DECIMAL_POINT;
                }
            }
        }
    }
...
static pv_status_t pv_normalizer_tagger_tag_measurement(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_MEASUREMENT]) {
        const char *string = token->string;
        ...
        if ((previous != NULL) &&
            ((previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL) ||
             (previous->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL) ||
             (previous->tag == PV_NORMALIZER_TAG_EN_DECIMAL_DIGITS))) {
            ...
            if (trie_index >= 0) {
                token->tag = PV_NORMALIZER_TAG_EN_MEASUREMENT;
        ...
        if ((previous != NULL) && (previous->tag == PV_NORMALIZER_TAG_EN_WORD)) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
            ...
            if (trie_index >= 0 && prev_trie_index >= 0) {
                ...
                previous->tag = PV_NORMALIZER_TAG_EN_MEASUREMENT;
                previous->tag_data_index = prev_trie_index;
                return PV_STATUS_SUCCESS;
            }
        } else if ((previous != NULL) && (previous->tag == PV_NORMALIZER_TAG_EN_DOT) &&
                   (two_previous != NULL) && (two_previous->tag == PV_NORMALIZER_TAG_EN_WORD)) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
            ...
            if (trie_index >= 0 && prev_trie_index >= 0) {
                ...
                two_previous->tag = PV_NORMALIZER_TAG_EN_MEASUREMENT;
        ...
        if (((previous != NULL) && (previous->tag == PV_NORMALIZER_TAG_EN_MEASUREMENT)) ||
            ((two_previous != NULL) && (two_previous->tag == PV_NORMALIZER_TAG_EN_MEASUREMENT))) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
        ...
        if ((length > 1) && (isdigit(string[0]) || ((string[0] == '-') && isdigit(string[1])))) {
            ...
            if ((split_index) && (split_index < length)) {
                ...
                if (trie_index >= 0) {
                    ...
                    status = pv_normalizer_tagger_en_tag_token(object, token, token_list);
                    if (status != PV_STATUS_SUCCESS) {
                        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                                pv_normalizer_tagger_en_tag_token,
                                pv_status_to_string(status));
                    ...
                    if (token->tag == PV_NORMALIZER_TAG_EN_CARDINAL && strlen(token->string) == 3) {
                        status = pv_normalizer_tagger_tag_comma_number(token, token_list);
                        if (status != PV_STATUS_SUCCESS) {
                            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                                    pv_normalizer_tagger_tag_comma_number,
                    ...
                    if (token->tag == PV_NORMALIZER_TAG_EN_CARDINAL ||
                        token->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL ||
                        token->tag == PV_NORMALIZER_TAG_EN_DECIMAL_DIGITS) {
                        measurement_token->tag = PV_NORMALIZER_TAG_EN_MEASUREMENT;
        ...
        if (length > 2) {
            ...
            if ((split_index > 0) && (split_index < length)) {
                ...
                if (trie_index >= 0) {
                    ...
                    if (next_previous->tag == PV_NORMALIZER_TAG_EN_MEASUREMENT) {
                        ...
                        next->tag = PV_NORMALIZER_TAG_EN_PER_SLASH;

                        next->next->tag = PV_NORMALIZER_TAG_EN_MEASUREMENT;
                        ...
                        if (trie_index_two >= 0) {
                            ...
                            next_previous->tag = PV_NORMALIZER_TAG_EN_MEASUREMENT;
    ...
    // `miles/hour`
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_PER_SLASH]) {
        ...
        if ((strcmp(token->string, "/") == 0) && (previous != NULL) && (next != NULL)) {
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS; i++) {
                if (strcmp(previous_upper_string, PV_NORMALIZER_MEASUREMENTS[i]) == 0) {
                    measurement_before = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS; i++) {
                if (strcmp(previous_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_PLURAL[i]) == 0) {
                    measurement_before = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS; i++) {
                if (strcmp(previous_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_SINGULAR[i]) == 0) {
                    measurement_before = true;
                    break;
                }
            }
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS; i++) {
                if (strcmp(next_upper_string, PV_NORMALIZER_MEASUREMENTS[i]) == 0) {
                    measurement_after = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS; i++) {
                if (strcmp(next_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_PLURAL[i]) == 0) {
                    measurement_after = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS; i++) {
                if (strcmp(next_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_SINGULAR[i]) == 0) {
                    measurement_after = true;
                    break;
                }
            }
            ...
            if (measurement_before && measurement_after) {
                token->tag = PV_NORMALIZER_TAG_EN_PER_SLASH;
            }
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_alphanum_spell_out(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_ALPHANUM_SPELL_OUT]) {
    ...
    if ((token->tag == PV_NORMALIZER_TAG_EN_ALPHANUM_SPELL_OUT) && (length >= 3)) {
        if (isdigit(string[length - 1]) || isdigit(string[length - 2])) {
            ...
            ad_bc_token->tag = PV_NORMALIZER_TAG_EN_LETTER_SPELL_OUT;
            token->tag = PV_NORMALIZER_TAG_EN_CARDINAL;
        }
    }
...
pv_status_t PV_MOCKABLE(pv_normalizer_tagger_en_tag_time)(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] >=
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_TIME_HOURS]) {
        return PV_STATUS_SUCCESS;
    }
    ...
    if (can_be_minutes) {
        ...
        bool has_hour_before_colon =
                (num_tokens_before > 1) &&
                (token->previous->previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL) &&
        ...
        if (has_colon_before_current && has_hour_before_colon) {
            ...
            if (smaller_than_61) {
                token->previous->previous->tag = PV_NORMALIZER_TAG_EN_TIME_HOURS;
                token->previous->tag = PV_NORMALIZER_TAG_EN_TIME_COLON;
                token->tag = PV_NORMALIZER_TAG_EN_TIME_MINUTES;
    ...
    if (num_tokens_before > 0) {
        if ((strcmp(token->string, "am") == 0) ||
            ...
            bool is_space_before = (token->previous->tag == PV_NORMALIZER_TAG_EN_SPACE);
            if (is_space_before) {
                pv_normalizer_token_t *two_previous = pv_normalizer_token_get_nth_token_before(token, 2, false);
                ...
                bool is_word_time_before_current = false;
                if ((two_previous != NULL) &&
                    (two_previous->tag == PV_NORMALIZER_TAG_EN_WORD)) {
                    char *upper_string = NULL;
                    pv_status_t status = pv_normalizer_util_upper(two_previous->string, &upper_string);
                    if (status != PV_STATUS_SUCCESS) {
                        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                                pv_normalizer_util_upper,
                                pv_status_to_string(status));
                    ...
                    for (int32_t i = 1; i < 20; i++) {
                        if (strcmp(upper_string, ONE_TO_NINETEEN[i]) == 0) {
                            is_word_time_before_current = true;
                        }
                    }
                    for (int32_t i = 1; i < 5; i++) {
                        if (strcmp(upper_string, TENS[i]) == 0) {
                            is_word_time_before_current = true;
                        }
                    }
                    free(upper_string);
                ...
                bool has_time_before_current =
                        ((two_previous != NULL) &&
                         ((two_previous->tag == PV_NORMALIZER_TAG_EN_TIME_HOURS) ||
                          (two_previous->tag == PV_NORMALIZER_TAG_EN_TIME_MINUTES))) ||
                        is_only_hour_time_before_current || is_word_time_before_current;
                if (has_time_before_current) {
                    token->tag = PV_NORMALIZER_TAG_EN_TIME_AM_PM;
    ...
    if (is_dot && (num_tokens_before > 4)) {
        ...
        if (is_am || is_pm) {
            ...
            bool is_only_hour_time_before_current =
                    (five_previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL) &&
                    !pv_normalizer_util_string_number_greater_than_int(five_previous->string, 12);
            bool has_time_before_current =
                    (five_previous->tag == PV_NORMALIZER_TAG_EN_TIME_HOURS) ||
                    (five_previous->tag == PV_NORMALIZER_TAG_EN_TIME_MINUTES) ||
        ...
        if (can_be_minutes_and_am_pm) {
            ...
            if (has_digits && (is_am || is_pm) && has_colon_before_current && has_hour_before_colon) {
                ...
                if (!pv_normalizer_util_string_number_greater_than_int(minute_string, 59)) {
                    ...
                    token->tag = PV_NORMALIZER_TAG_EN_TIME_MINUTES;
                    am_pm_token->tag = PV_NORMALIZER_TAG_EN_TIME_AM_PM;
                    token->previous->tag = PV_NORMALIZER_TAG_EN_TIME_COLON;
                    token->previous->previous->tag = PV_NORMALIZER_TAG_EN_TIME_HOURS;
    ...
    if ((strlen(token->string) >= 3) && pv_normalizer_token_is_space_or_null(token->previous)) {
        ...
        if (found_time) {
            token->tag = PV_NORMALIZER_TAG_EN_TIME_HOURS;
            pv_normalizer_token_t *am_pm_token = token->next;
            am_pm_token->tag = PV_NORMALIZER_TAG_EN_TIME_AM_PM;
        ...
        if (three_previous_can_be_minutes_and_am_pm && previous_is_m && two_previous_is_dot && no_spaces) {
            ...
            if (has_digits && (is_am || is_pm)) {
                ...
                bool has_hour_four_before_colon =
                        ...
                        (four_previous->previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL) &&
                ...
                if (has_hour_four_before_colon) {
                    ...
                    if (!pv_normalizer_util_string_number_greater_than_int(minute_string, 59)) {
                        ...
                        minute_token->tag = PV_NORMALIZER_TAG_EN_TIME_MINUTES;
                        am_pm_token->tag = PV_NORMALIZER_TAG_EN_TIME_AM_PM;
                        time_colon_token->tag = PV_NORMALIZER_TAG_EN_TIME_COLON;
                        hour_token->tag = PV_NORMALIZER_TAG_EN_TIME_HOURS;
        ...
        if (three_previous_can_be_minutes_and_am_pm && previous_is_m && two_previous_is_dot && no_spaces) {
            ...
            if (has_digits && (is_am || is_pm)) {
                ...
                if (!pv_normalizer_util_string_number_greater_than_int(hour_string, 12)) {
                    ...
                    pv_normalizer_token_t *am_pm_token = three_previous->next;
                    three_previous->tag = PV_NORMALIZER_TAG_EN_TIME_HOURS;
                    am_pm_token->tag = PV_NORMALIZER_TAG_EN_TIME_AM_PM;
...
static pv_status_t merge_comma_number_tokens(
    ...
    pv_normalizer_token_tag_en_t tag = first->tag;
    if (last->tag == PV_NORMALIZER_TAG_EN_ORDINAL) {
        if (first->tag == PV_NORMALIZER_TAG_EN_CARDINAL) {
            tag = PV_NORMALIZER_TAG_EN_ORDINAL;
        } else {
            tag = PV_NORMALIZER_TAG_EN_NEGATIVE_ORDINAL;
        }
    } else if (last->tag == PV_NORMALIZER_TAG_EN_CURRENCY) {
        if (first->tag == PV_NORMALIZER_TAG_EN_CARDINAL) {
            tag = PV_NORMALIZER_TAG_EN_CURRENCY;
        } else {
            tag = PV_NORMALIZER_TAG_EN_NEGATIVE_CURRENCY;
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_comma_number(pv_normalizer_token_t *token, pv_normalizer_token_list_t *token_list) {
    ...
    while (!is_first_token) {
        ...
        if (current_is_cardinal) {
            ...
            if ((current->tag == PV_NORMALIZER_TAG_EN_CARDINAL) &&
                ((is_first_token && (strlen(string) <= 3)) ||
                 (!is_first_token && (strlen(string) == 3)))) {
                current_is_cardinal = true;
            } else if (is_first_token &&
                       (current->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL) &&
                       (strlen(string) <= 4)) {
                current_is_cardinal = true;
            } else if (is_first_token &&
                       (current->tag == PV_NORMALIZER_TAG_EN_CURRENCY)) {
                char character[PV_NORMALIZER_MAX_NUM_BYTES_PER_CHARACTER] = {0};
                int32_t num_bytes_character = 0;
                ...
                if ((strlen(string) - num_bytes_character) <= 3) {
                    current_is_cardinal = true;
                       ...
                       (current->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CURRENCY)) {
                char character[PV_NORMALIZER_MAX_NUM_BYTES_PER_CHARACTER] = {0};
                int32_t num_bytes_character = 0;
...
static pv_status_t pv_normalizer_tagger_tag_fraction(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_FRACTION_SLASH]) {
        const char *string = token->string;
        ...
        if (is_fraction && (num_slashes == 1) && (num_cardinals == 2)) {
            ...
            status = pv_normalizer_tagger_en_tag_token(object, token->next->next, token_list);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                        pv_normalizer_tagger_en_tag_token,
                        pv_status_to_string(status));
            ...
            bool after_slash_is_number =
                    ((token->next->next->tag == PV_NORMALIZER_TAG_EN_CARDINAL) ||
                     (token->next->next->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CARDINAL) ||
                     (token->next->next->tag == PV_NORMALIZER_TAG_EN_DECIMAL_DIGITS));

            if (before_slash_is_number && after_slash_is_number) {
                token->next->tag = PV_NORMALIZER_TAG_EN_FRACTION_SLASH;
            }
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_url(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DOT]) {
        const char *string = token->string;
        ...
        if ((strcmp(string, ".") == 0) &&
            ...
            token->tag = PV_NORMALIZER_TAG_EN_DOT;
        }
    }

    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_COLON]) {
        const char *string = token->string;
        ...
        if ((strcmp(string, ":") == 0) &&
            previous_is_non_space_character &&
            next_is_non_space_character) {
            token->tag = PV_NORMALIZER_TAG_EN_COLON;
        }
    }

    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_ACRONYM]) {
        char *upper_string = NULL;
        ...
        for (int32_t i = 0; i < PV_NORMALIZER_NUM_ACRONYMS; i++) {
            if (strcmp(upper_string, PV_NORMALIZER_ACRONYMS[i]) == 0) {
                token->tag = PV_NORMALIZER_TAG_EN_ACRONYM;
                break;
            }
        }
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_TOP_LEVEL_DOMAIN]) {
        if ((token->previous != NULL) &&
            !token->previous->next_character_is_space &&
            (token->previous->tag == PV_NORMALIZER_TAG_EN_DOT || strcmp(token->previous->string, ".") == 0)) {
            char *upper_string = NULL;
            pv_status_t status = pv_normalizer_util_upper(token->string, &upper_string);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_TOP_LEVEL_DOMAINS; i++) {
                if (strcmp(upper_string, PV_NORMALIZER_TOP_LEVEL_DOMAINS[i]) == 0) {
                    token->tag = PV_NORMALIZER_TAG_EN_TOP_LEVEL_DOMAIN;
                    token->tag_data_index = i;
                    if (token->previous->tag != PV_NORMALIZER_TAG_EN_DOT) {
                        token->previous->tag = PV_NORMALIZER_TAG_EN_DOT;
                    }
                    break;
                }
            }
...
static pv_status_t pv_normalizer_tagger_tag_currency(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_CURRENCY]) {
        const char *string = token->string;
        int32_t length = (int32_t) strlen(string);
        ...
        for (int32_t i = 0; i < PV_NORMALIZER_NUM_CURRENCIES; i++) {
            num_bytes_currency = (int32_t) strlen(PV_NORMALIZER_CURRENCIES[i]);
            if (strncmp(string, PV_NORMALIZER_CURRENCIES[i], num_bytes_currency) == 0) {
                currency_index = i;
        ...
        if (starts_with_currency && (num_bytes_currency != length)) {
            ...
            if (only_digits) {
                token->tag = PV_NORMALIZER_TAG_EN_CURRENCY;
        ...
        if (isdigit(string[0])) {
            ...
            while (i < length) {
                ...
                if (!isdigit(string[i])) {
                    for (int32_t j = 0; j < PV_NORMALIZER_NUM_CURRENCIES; j++) {
                        num_bytes_currency = (int32_t) strlen(PV_NORMALIZER_CURRENCIES[j]);
                        if (strncmp(string + i, PV_NORMALIZER_CURRENCIES[j], num_bytes_currency) == 0) {
            ...
            if (only_digits && ends_with_currency) {
                ...
                if ((token->previous != NULL) && token->previous->previous != NULL) {
                    ...
                    if (is_currency_decimal) {
                        pv_normalizer_token_tag_en_t two_previous_tag = two_previous->tag;
                        ...
                        if (two_previous_tag == PV_NORMALIZER_TAG_EN_CARDINAL) {
                            merged_token->tag = PV_NORMALIZER_TAG_EN_CURRENCY;
                        } else {
                            merged_token->tag = PV_NORMALIZER_TAG_EN_NEGATIVE_CURRENCY;
                        }
        ...
        if (only_digits && (token->previous != NULL) && (token->previous->previous != NULL)) {
            ...
            bool is_currency_decimal =
                    (strcmp(previous->string, ".") == 0) &&
                    (!previous->next_character_is_space) &&
                    ((two_previous->tag == PV_NORMALIZER_TAG_EN_CURRENCY) ||
                     (two_previous->tag == PV_NORMALIZER_TAG_EN_NEGATIVE_CURRENCY)) &&
                    (!two_previous->next_character_is_space);
        ...
        if (string[0] == '-') {
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_CURRENCIES; i++) {
                num_bytes_currency = (int32_t) strlen(PV_NORMALIZER_CURRENCIES[i]);
                if (strncmp(string + 1, PV_NORMALIZER_CURRENCIES[i], num_bytes_currency) == 0) {
                    currency_index = i;
            ...
            if (second_character_is_currency) {
                ...
                if (only_digits) {
                    token->tag = PV_NORMALIZER_TAG_EN_NEGATIVE_CURRENCY;
                    token->tag_data_index = currency_index;
                    return PV_STATUS_SUCCESS;
                }
            }
        ...
        for (int32_t i = 0; i < PV_NORMALIZER_NUM_CURRENCIES; i++) {
            if (strcmp(string, PV_NORMALIZER_CURRENCIES[i]) == 0) {
                currency_index = i;
                is_currency = true;
                break;
            }
        ...
        if (is_currency && (previous_is_cardinal || previous_is_decimal)) {
            token->tag = PV_NORMALIZER_TAG_EN_CURRENCY_SYMBOL;
            token->tag_data_index = currency_index;
            return PV_STATUS_SUCCESS;
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_abbreviation(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_ABBREVIATION]) {
        bool is_dot = (strcmp(token->string, ".") == 0);
        if (is_dot) {
            pv_normalizer_token_t *start_token = pv_normalizer_token_get_token_after_previous_space(token);
            while (start_token &&
                   (start_token->tag == PV_NORMALIZER_TAG_EN_SPECIAL_CHARACTER ||
                    start_token->tag == PV_NORMALIZER_TAG_EN_SINGLE_QUOTE) &&
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_ABBREVIATIONS; i++) {
                if (strcmp(potential_abbreviation, PV_NORMALIZER_ABBREVIATIONS[i]) == 0) {
                    token->tag = PV_NORMALIZER_TAG_EN_ABBREVIATION;
                    token->tag_data_index = i;
                    break;
                }
            }
            ...
            if (token->tag == PV_NORMALIZER_TAG_EN_ABBREVIATION) {
                ...
                if (token->next == NULL || token->next_character_is_space) {
                    abbreviation_original_string = (char *) calloc(strlen(start_token->original_string) +
...
pv_status_t pv_normalizer_tagger_tag_digits_sequence(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DIGITS_WITH_PARENTHESES]) {
        ...
        if ((length == 5) && (string[0] == '(') && (string[4] == ')')) {
            if (isdigit(string[1]) && isdigit(string[2]) && isdigit(string[3])) {
                token->tag = PV_NORMALIZER_TAG_EN_DIGITS_WITH_PARENTHESES;
            }
        }
    }

    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DIGITS]) {
        ...
        if ((previous_no_skip_space != NULL) && (strcmp(previous_no_skip_space->string, "+") == 0)) {
            if (pv_normalizer_util_only_contains_digits(string)) {
                token->tag = PV_NORMALIZER_TAG_EN_DIGITS;
                return PV_STATUS_SUCCESS;
            }
        }
        ...
        if (is_digits_with_dashes) {
            ...
            while (current != NULL) {
                ...
                if ((strlen(current->string) == 1) && pv_normalizer_tagger_is_digit_separator(current->string[0])) {
                    current->tag = PV_NORMALIZER_TAG_EN_DIGITS_SEPARATOR;
                } else {
                    current->tag = PV_NORMALIZER_TAG_EN_DIGITS;
                }
        ...
        bool is_second_number_in_digits_sequence =
                ...
                (previous_skip_space->tag == PV_NORMALIZER_TAG_EN_DIGITS_WITH_PARENTHESES);
        pv_normalizer_token_t *two_previous = pv_normalizer_token_get_nth_token_before(token, 2, false);
        pv_normalizer_token_t *four_previous = pv_normalizer_token_get_nth_token_before(token, 4, false);
        bool is_third_number_in_digits_sequence =
                (four_previous != NULL) &&
                (four_previous->tag == PV_NORMALIZER_TAG_EN_DIGITS_WITH_PARENTHESES) &&
                (two_previous->tag == PV_NORMALIZER_TAG_EN_DIGITS);
...
int32_t pv_normalizer_tagger_get_month_index(const char *string) {
    ...
    for (int32_t i = 0; i < PV_NORMALIZER_NUM_MONTH_NAMES; i++) {
        if (strcmp(string, PV_NORMALIZER_MONTH_NAMES[i]) == 0) {
            return i;
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_date(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DATE_MONTH]) {
        const char *string = token->string;
        int32_t length = (int32_t) strlen(string);
        ...
        if (index_month_name >= 0) {
            token->tag = PV_NORMALIZER_TAG_EN_DATE_MONTH;
            token->tag_data_index = index_month_name;

            pv_normalizer_token_t *previous = pv_normalizer_token_get_nth_token_before(token, 1, true);
            if (previous != NULL &&
                previous->tag == PV_NORMALIZER_TAG_EN_CARDINAL &&
                !pv_normalizer_util_string_number_greater_than_int(previous->string, 31)) {
                previous->tag = PV_NORMALIZER_TAG_EN_DATE_DAY;
            }
        ...
        if (current_is_year) {
            ...
            if (is_date) {
                token->tag = PV_NORMALIZER_TAG_EN_DATE_YEAR;
                previous->tag = PV_NORMALIZER_TAG_EN_DATE_SEPARATOR;
                three_previous->tag = PV_NORMALIZER_TAG_EN_DATE_SEPARATOR;

                if (two_previous_is_month && four_previous_is_day) {
                    two_previous->tag = PV_NORMALIZER_TAG_EN_DATE_MONTH;
                    four_previous->tag = PV_NORMALIZER_TAG_EN_DATE_DAY;
                } else {
                    two_previous->tag = PV_NORMALIZER_TAG_EN_DATE_DAY;
                    four_previous->tag = PV_NORMALIZER_TAG_EN_DATE_MONTH;
                }
        ...
        if (four_previous_is_year) {
            ...
            bool is_date =
                    ...
                    current_is_day;
            if (is_date) {
                four_previous->tag = PV_NORMALIZER_TAG_EN_DATE_YEAR;
                three_previous->tag = PV_NORMALIZER_TAG_EN_DATE_SEPARATOR;
                two_previous->tag = PV_NORMALIZER_TAG_EN_DATE_MONTH;
                previous->tag = PV_NORMALIZER_TAG_EN_DATE_SEPARATOR;
                token->tag = PV_NORMALIZER_TAG_EN_DATE_DAY;
        ...
        bool current_is_day =
                ...
                pv_normalizer_util_string_number_greater_than_int(token->string, 0) &&
                (previous_skip_space != NULL) &&
                (previous_skip_space->tag == PV_NORMALIZER_TAG_EN_DATE_MONTH);
        if (current_is_day) {
            token->tag = PV_NORMALIZER_TAG_EN_DATE_DAY;
        }
        ...
        current_is_year =
                ...
                (strlen(string) == 4) &&
                (((previous_skip_space != NULL) &&
                  (previous_skip_space->tag == PV_NORMALIZER_TAG_EN_DATE_DAY)) ||
                 ((two_previous_skip_space != NULL) &&
                  (two_previous_skip_space->tag == PV_NORMALIZER_TAG_EN_DATE_DAY))) &&
                ((three_previous != NULL) &&
                 (three_previous->tag != PV_NORMALIZER_TAG_EN_DATE_SEPARATOR));
        if (current_is_year) {
            token->tag = PV_NORMALIZER_TAG_EN_DATE_YEAR;
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DATE_YEAR]) {
        // to tag the "2020", in "June 2020" or "June 1st, 2020"
        const char *string = token->string;
        int32_t length = (int32_t) strlen(string);
        ...
        if (current_can_be_year) {
            pv_normalizer_token_t *previous = pv_normalizer_token_get_nth_token_before(token, 1, true);
            bool previous_is_month = previous != NULL && previous->tag == PV_NORMALIZER_TAG_EN_DATE_MONTH;
            if (previous_is_month) {
                token->tag = PV_NORMALIZER_TAG_EN_DATE_YEAR;
                return PV_STATUS_SUCCESS;
            }
            ...
            if (previous != NULL && strcmp(previous->string, ",") == 0) {
                ...
                bool two_previous_is_day = two_previous != NULL && two_previous->tag == PV_NORMALIZER_TAG_EN_DATE_DAY;
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_DATE_DAY]) {
        // to tag the "1st", in "June 1st"
        pv_normalizer_token_t *previous = pv_normalizer_token_get_nth_token_before(token, 1, true);
        bool previous_is_month = previous != NULL && previous->tag == PV_NORMALIZER_TAG_EN_DATE_MONTH;
        if (!previous_is_month) {
            return PV_STATUS_SUCCESS;
        ...
        if (length >= 3) {
            ...
            if (!found_non_digit) {
                ...
                if (is_valid_ending) {
                    ...
                    if (!pv_normalizer_util_string_number_greater_than_int(number_string, 31)) {
                        current_can_be_day = true;
                    }
                    free(number_string);
                }
            }
...
pv_status_t pv_normalizer_tagger_tag_name(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_EN_WEIGHTS[PV_NORMALIZER_TAG_EN_NAME_INITIAL_LETTER]) {
        const char *string = token->string;
        ...
        if (is_dot) {
            ...
            if (previous_token != NULL) {
                ...
                if (is_capitalized && (strlen(previous_token->string) == 1)) {
                    previous_token->tag = PV_NORMALIZER_TAG_EN_NAME_INITIAL_LETTER;
                    token->tag = PV_NORMALIZER_TAG_EN_NAME_INITIAL_DOT;
                }
            }
        }
    }
...
pv_status_t pv_normalizer_tagger_en_merge_word_token(pv_normalizer_token_list_t *token_list) {
    PV_ASSERT(token_list);

    pv_normalizer_token_t *current = token_list->head;
    while (current) {
        if (current->tag == PV_NORMALIZER_TAG_EN_WORD || current->tag == PV_NORMALIZER_TAG_EN_SINGLE_QUOTE) {
            ...
            while ((last->next) && (!last->next_character_is_space) && (!last->next_character_is_hyphen)) {
                if (last->next->tag == PV_NORMALIZER_TAG_EN_WORD || last->next->tag == PV_NORMALIZER_TAG_EN_SINGLE_QUOTE) {
                    last = last->next;
            ...
            if (first != last) {
                bool merge_original = (last->tag == PV_NORMALIZER_TAG_EN_SINGLE_QUOTE &&
                                       (last->next_character_is_space ||
                                        last->next == NULL ||
                                        (last->next && last->next->tag == PV_NORMALIZER_TAG_EN_SPACE)));
                ...
                merged_token->tag = PV_NORMALIZER_TAG_EN_WORD;
