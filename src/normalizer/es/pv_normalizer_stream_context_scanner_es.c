...
bool PV_MOCKABLE(pv_normalizer_stream_context_scanner_es_is_verbalizable)(
    ...
    if (token->tag == PV_NORMALIZER_TAG_ES_SPECIAL_CHARACTER) {
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (use_cases[j] == PV_NORMALIZER_USE_DIGITS_SEQUENCE_NORMALIZER_ES) {
                if (pv_normalizer_stream_context_can_be_phone_number(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_FRACTION_NORMALIZER_ES) {
    ...
    if (token->tag == PV_NORMALIZER_TAG_ES_SPACE) {
    ...
    if (token->tag == PV_NORMALIZER_TAG_ES_PUNCTUATION) {
        ...
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (
                    (use_cases[j] == PV_NORMALIZER_USE_URL_NORMALIZER_ES) ||
                    use_cases[j] == PV_NORMALIZER_USE_ABBREVIATION_NORMALIZER_ES) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_TIME_NORMALIZER_ES) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_time(token)) {
                    return false;
                }
            } else if (
                    (use_cases[j] == PV_NORMALIZER_USE_CARDINAL_NORMALIZER_ES) ||
                    use_cases[j] == PV_NORMALIZER_USE_NEGATIVE_CARDINAL_NORMALIZER_ES) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_cardinal(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_DECIMAL_NORMALIZER_ES) {
...
void PV_MOCKABLE(pv_normalizer_stream_context_scanner_es_remove_hyphen_only_tokens)(
    ...
    while (current) {
        ...
        if (current->tag != PV_NORMALIZER_TAG_ES_NUMBER_RANGE_TO) {
...
pv_status_t pv_normalizer_stream_context_can_be_time(const pv_normalizer_token_t *token) {
    ...
    return (((token->tag == PV_NORMALIZER_TAG_ES_CARDINAL) &&
            ...
            ((token->tag == PV_NORMALIZER_TAG_ES_ALPHANUM_SPELL_OUT) && (strlen(token->string) <= 3)));
}
...
bool pv_normalizer_stream_context_is_punctuation_part_of_time(const pv_normalizer_token_t *token) {
    ...
    bool can_be_time =
            ...
            (token->previous != NULL) &&
            (token->previous->tag == PV_NORMALIZER_TAG_ES_CARDINAL) &&
...
bool pv_normalizer_stream_context_is_punctuation_part_of_cardinal(const pv_normalizer_token_t *token) {
    ...
    return
            ...
            ((token->previous != NULL) &&
             ((token->previous->tag == PV_NORMALIZER_TAG_ES_CARDINAL) ||
              (token->previous->tag == PV_NORMALIZER_TAG_ES_NEGATIVE_CARDINAL)));
}
...
bool pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(const pv_normalizer_token_t *token) {
    ...
    bool previous_is_punctuation = (token->previous != NULL) && (token->previous->tag == PV_NORMALIZER_TAG_ES_PUNCTUATION);
