...
bool PV_MOCKABLE(pv_normalizer_stream_context_scanner_ja_is_verbalizable)(
    ...
    if (token->tag == PV_NORMALIZER_TAG_JA_SPECIAL_CHARACTER) {
        for (int32_t j = 0; j < num_use_cases; j++) {
            if (use_cases[j] == PV_NORMALIZER_USE_DIGITS_SEQUENCE_NORMALIZER_JA) {
                if (pv_normalizer_stream_context_can_be_phone_number(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_FRACTION_NORMALIZER_JA) {
    ...
    if (token->tag == PV_NORMALIZER_TAG_JA_TIME_HOURS || token->tag == PV_NORMALIZER_TAG_JA_TIME_MINUTES) {
        return false;
    }

    if (token->tag == PV_NORMALIZER_TAG_JA_TIME_AM_PM) {
        return true;
    }
    ...
    if (token->tag == PV_NORMALIZER_TAG_JA_SPACE) {
        if (token->previous != NULL &&
            (token->previous->tag == PV_NORMALIZER_TAG_JA_TIME_HOURS ||
             token->previous->tag == PV_NORMALIZER_TAG_JA_TIME_MINUTES ||
             token->previous->tag == PV_NORMALIZER_TAG_JA_DATE_MONTH ||
             token->previous->tag == PV_NORMALIZER_TAG_JA_CARDINAL ||
             token->previous->tag == PV_NORMALIZER_TAG_JA_MEASUREMENT ||
             token->previous->tag == PV_NORMALIZER_TAG_JA_DATE_DAY)) {
            return false;
        }
        return true;
    }
    ...
    if (token->tag == PV_NORMALIZER_TAG_JA_PUNCTUATION) {
        ...
        for (int32_t j = 0; j < num_use_cases; j++) {
            if ((use_cases[j] == PV_NORMALIZER_USE_URL_NORMALIZER_JA) ||
                (use_cases[j] == PV_NORMALIZER_USE_ABBREVIATION_NORMALIZER_JA)) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_TIME_NORMALIZER_JA) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_time(token)) {
                    return false;
                }
            } else if ((use_cases[j] == PV_NORMALIZER_USE_CARDINAL_NORMALIZER_JA) ||
                       (use_cases[j] == PV_NORMALIZER_USE_NEGATIVE_CARDINAL_NORMALIZER_JA)) {
                if (pv_normalizer_stream_context_is_punctuation_part_of_cardinal(token)) {
                    return false;
                }
            } else if (use_cases[j] == PV_NORMALIZER_USE_DECIMAL_NORMALIZER_JA) {
...
void PV_MOCKABLE(pv_normalizer_stream_context_scanner_ja_remove_hyphen_only_tokens)(
    ...
    while (current) {
        ...
        if (current->tag != PV_NORMALIZER_TAG_JA_NUMBER_RANGE_TO) {
...
pv_status_t pv_normalizer_stream_context_can_be_time(const pv_normalizer_token_t *token) {
    ...
    return (((token->tag == PV_NORMALIZER_TAG_JA_CARDINAL) &&
            ...
            ((token->tag == PV_NORMALIZER_TAG_JA_ALPHANUM_SPELL_OUT) && (strlen(token->string) <= 3)));
}
...
bool pv_normalizer_stream_context_is_punctuation_part_of_cardinal(const pv_normalizer_token_t *token) {
    ...
    return (strcmp(token->string, ",") == 0) &&
           ((token->previous != NULL) &&
            ((token->previous->tag == PV_NORMALIZER_TAG_JA_CARDINAL) ||
             (token->previous->tag == PV_NORMALIZER_TAG_JA_NONE) ||
             (token->previous->tag == PV_NORMALIZER_TAG_JA_NEGATIVE_CARDINAL)));
}
...
bool pv_normalizer_stream_context_is_punctuation_part_of_url_or_abbreviation(const pv_normalizer_token_t *token) {
    ...
    bool previous_is_punctuation = (token->previous != NULL) && (token->previous->tag == PV_NORMALIZER_TAG_JA_PUNCTUATION);
