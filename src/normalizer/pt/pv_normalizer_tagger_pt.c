...
struct pv_normalizer_tagger_pt {
...
static pv_status_t pv_normalizer_tagger_pt_tag_postprocess(pv_normalizer_token_list_t *token_list);
...
pv_status_t pv_normalizer_tagger_get_noun_gender(
        const char *noun,
        const pv_noun_gender_dict_t *noun_gender_dict,
        pv_noun_gender_t *gender);

static pv_status_t pv_normalizer_tagger_tag_cardinal(
        const pv_normalizer_tagger_pt_t *object,
        pv_normalizer_token_t *token,
...
// TODO: remove the following once changed to mockable
static pv_status_t pv_normalizer_tagger_pt_tag_time(
        const pv_normalizer_tagger_pt_t *object,
        pv_normalizer_token_t *token,
        pv_normalizer_token_list_t *token_list);
...
static bool pv_normalizer_is_ordinal_too_large(const pv_normalizer_token_t *token);
...
// TODO: remove the following once changed to mockable
pv_status_t pv_normalizer_tagger_pt_split_untagged_token(
        const pv_normalizer_tagger_pt_t *object,
        pv_normalizer_token_t **token,
        pv_normalizer_token_list_t **token_list,
        bool *did_split);

static pv_status_t pv_normalizer_tagger_pt_merge_word_token(pv_normalizer_token_list_t *token_list);

pv_status_t PV_MOCKABLE(pv_normalizer_tagger_pt_init)(
        const pv_language_info_t *language_info,
        int32_t num_use_cases,
        const pv_normalizer_use_cases_pt_t *use_cases,
        pv_normalizer_tokenizer_t *tokenizer,
        pv_noun_gender_dict_t *noun_gender_dict,
        pv_normalizer_tagger_pt_t **object) {
    ...
    *object = NULL;
    ...
    pv_normalizer_tagger_pt_t *o = calloc(1, sizeof(pv_normalizer_tagger_pt_t));
    if (!o) {
        PV_ERROR_REPORT(
                &pv_error_msg_alloc,
    ...
    for (int32_t i = 0; i < num_use_cases; i++) {
        if (use_cases[i] == PV_NORMALIZER_USE_MEASUREMENT_NORMALIZER_PT) {
            // TODO (Tina): Below line is passing the wrong <num_normalizable_characters> which creates more children because in TRIE we only use upper case,
            // TODO: but this doesn't cause bug for now, so leave it for future refactoring. Do the same for other languages.
            status = pv_normalizer_util_trie_create(
                    num_normalizable_characters,
                    normalizable_characters,
                    PV_ARRAY_LEN(PV_NORMALIZER_MEASUREMENTS_PT),
                    (const char **) PV_NORMALIZER_MEASUREMENTS_PT,
                    &(o->measurement_trie));
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
...
void PV_MOCKABLE(pv_normalizer_tagger_pt_delete)(pv_normalizer_tagger_pt_t *object) {
    if (object) {
...
static pv_status_t pv_normalizer_tagger_pt_tag_token(
        const pv_normalizer_tagger_pt_t *object,
        pv_normalizer_token_t *token,
        pv_normalizer_token_list_t *token_list) {
    PV_ASSERT(object);
    ...
    for (int32_t j = 0; j < object->num_use_cases; j++) {
        ...
        if (object->use_cases[j] == PV_NORMALIZER_USE_WORD_NORMALIZER_PT) {
            ...
            pv_status_t status = pv_normalizer_tagger_pt_tag_time(object, token, token_list);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                        pv_normalizer_tagger_pt_tag_time,
                        pv_status_to_string(status));
                return status;
            }
...
pv_status_t PV_MOCKABLE(pv_normalizer_tagger_pt_tag)(
        pv_normalizer_tagger_pt_t *object,
        pv_normalizer_token_list_t *token_list,
    ...
    while (token != NULL) {
        ...
        pv_status_t status = pv_normalizer_tagger_pt_tag_token(object, token, token_list);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                    pv_normalizer_tagger_pt_tag_token,
                    pv_status_to_string(status));
            return status;
        }

        bool did_split = false;
        if (split_untagged && (token->tag == PV_NORMALIZER_TAG_PT_NONE)) {
            pv_normalizer_token_list_t *split_token_list = NULL;

            status = pv_normalizer_tagger_pt_split_untagged_token(
                    object,
                    &token,
                    &split_token_list,
                    &did_split);
    ...
    pv_status_t status = pv_normalizer_tagger_pt_tag_postprocess(token_list);
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_normalizer_tagger_pt_tag_postprocess,
                pv_status_to_string(status));
        return status;
    }
...
pv_status_t pv_normalizer_tagger_pt_tag_postprocess(pv_normalizer_token_list_t *token_list) {
    PV_ASSERT(token_list);

    pv_status_t status = pv_normalizer_tagger_pt_merge_word_token(token_list);
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_normalizer_tagger_pt_merge_word_token,
                pv_status_to_string(status));
        return status;
    }
    ...
    while (token != NULL) {
        bool is_spell_out =
                (token->tag == PV_NORMALIZER_TAG_PT_ALPHANUM_SPELL_OUT) ||
                (token->tag == PV_NORMALIZER_TAG_PT_LETTER_SPELL_OUT) ||
                (token->tag == PV_NORMALIZER_TAG_PT_ACRONYM) ||
                (token->tag == PV_NORMALIZER_TAG_PT_NAME_INITIAL_LETTER) ||
                ((token->tag == PV_NORMALIZER_TAG_PT_TOP_LEVEL_DOMAIN) &&
                 PV_NORMALIZER_TOP_LEVEL_DOMAINS_IS_SPELL_OUT_PT[token->tag_data_index]);
        if (is_spell_out) {
            ...
            while (strlen(current->string) >= 2) {
                ...
                if (isdigit(current->string[0])) {
                    current->tag = PV_NORMALIZER_TAG_PT_CARDINAL_SPELL_OUT;
                } else {
                    current->tag = PV_NORMALIZER_TAG_PT_LETTER_SPELL_OUT;
                }
...
pv_status_t pv_normalizer_tagger_tag_word(const pv_language_info_t *language_info, pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_WORD]) {
        pv_status_t status = pv_normalizer_tagger_tag_word_common(language_info, token, PV_NORMALIZER_TAG_PT_WORD);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
...
pv_status_t pv_normalizer_tagger_tag_punctuation(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_PUNCTUATION]) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_SINGLE_QUOTE]) {
        if (strcmp(token->string, APOSTROPHE) == 0) {
            token->tag = PV_NORMALIZER_TAG_PT_SINGLE_QUOTE;
        }
    }
...
void pv_normalizer_tagger_tag_custom_pronunciation(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_CUSTOM_PRONUNCIATION]) {
        if (token->pronunciation != NULL) {
            token->tag = PV_NORMALIZER_TAG_PT_CUSTOM_PRONUNCIATION;
        }
    }
}

// TODO: Change mockable
pv_status_t pv_normalizer_tagger_pt_split_untagged_token(
        const pv_normalizer_tagger_pt_t *object,
        pv_normalizer_token_t **token,
        pv_normalizer_token_list_t **token_list,
    ...
    *did_split = false;
    ...
    if ((*token)->tag == PV_NORMALIZER_TAG_PT_NONE) {
...
pv_status_t pv_normalizer_tagger_get_noun_gender(
    ...
    char *token_upper = NULL;
    // TODO(Eric Mikulin): Replace magic `4` with constant when merged in
    token_upper = calloc((strlen(noun) * 4) + 1, sizeof(char));
    if (!token_upper) {
        PV_ERROR_REPORT(
                &pv_error_msg_alloc,
                PV_ERROR_ARGS_PUBLIC_EMPTY(),
                PV_ERROR_ARGS_PRIVATE("token_upper"));
        return PV_STATUS_OUT_OF_MEMORY;
    }

    strcpy(token_upper, noun);
    pv_status_t status = pv_normalizer_util_upper_inplace(token_upper);
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
    ...
    const pv_noun_gender_t nouns_gender = pv_noun_gender_dict_gender(noun_gender_dict, token_upper);
    *gender = nouns_gender;
    free(token_upper);
    return PV_STATUS_SUCCESS;
}
...
pv_status_t pv_normalizer_tagger_tag_cardinal(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_CARDINAL]) {
        const char *string = token->string;

        if (pv_normalizer_util_only_contains_digits(string)) {
            token->tag = PV_NORMALIZER_TAG_PT_CARDINAL;
            ...
            bool could_be_fraction_numerator = (!token->next_character_is_space &&
                                                token->next != NULL && token->next->string[0] == '/');
            ...
            if ((token->next_character_is_space ||
                ...
                (strlen(string) == 3)) {
                // pt use dot to separate thousands
    ...
    if (strlen(token->string) > 0 &&
        (token->tag != PV_NORMALIZER_TAG_PT_CARDINAL &&
         token->tag != PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL)) {
        pv_noun_gender_t gender = PV_NOUN_GENDER_NO_MATCH;
        pv_status_t status = pv_normalizer_tagger_get_noun_gender(token->string, object->noun_gender_dict, &gender);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                    get_noun_gender,
                    pv_status_to_string(status));
        ...
        if (gender == PV_NOUN_GENDER_NO_MATCH) {
            token->gender = PV_NORMALIZER_TOKEN_GENDER_NONE;
            return PV_STATUS_SUCCESS;
        }
        ...
        if (previous != NULL &&
            (previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL ||
             previous->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL)) {

            if (gender == PV_NOUN_GENDER_FEMININE) {
                previous->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
            } else {
                previous->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
            }

            pv_normalizer_token_t *two_previous = pv_normalizer_token_get_nth_token_before(previous, 1, false);
            pv_normalizer_token_t *three_previous = pv_normalizer_token_get_nth_token_before(previous, 2, false);
            if (two_previous != NULL && three_previous != NULL &&
                two_previous->tag == PV_NORMALIZER_TAG_PT_NUMBER_RANGE_TO &&
                three_previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL) {
                if (gender == PV_NOUN_GENDER_FEMININE) {
                    three_previous->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                } else {
                    three_previous->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
                }
            }
...
void pv_normalizer_tagger_tag_negative_cardinal(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL]) {
        const char *string = token->string;
...
static pv_status_t merge_dot_number_tokens(
    ...
    pv_normalizer_token_tag_pt_t tag = first->tag;
    pv_normalizer_token_gender_t gender = last->gender;

    if (last->tag == PV_NORMALIZER_TAG_PT_ORDINAL) {
        if (first->tag == PV_NORMALIZER_TAG_PT_CARDINAL) {
            tag = PV_NORMALIZER_TAG_PT_ORDINAL;
        } else {
            tag = PV_NORMALIZER_TAG_PT_NEGATIVE_ORDINAL;
        }
    } else if (last->tag == PV_NORMALIZER_TAG_PT_CURRENCY) {
        if (first->tag == PV_NORMALIZER_TAG_PT_CARDINAL) {
            tag = PV_NORMALIZER_TAG_PT_CURRENCY;
        } else {
            tag = PV_NORMALIZER_TAG_PT_NEGATIVE_CURRENCY;
        }
    } else if (first->tag == PV_NORMALIZER_TAG_PT_CURRENCY ||
               first->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CURRENCY) {
        gender = first->gender;
    }
    ...
    char *combined_string = calloc(combined_string_length + 1, sizeof(char));
    if (!combined_string) {
        PV_ERROR_REPORT(
                &pv_error_msg_alloc,
                PV_ERROR_ARGS_PUBLIC_EMPTY(),
                PV_ERROR_ARGS_PRIVATE("combined_string"));
        return PV_STATUS_OUT_OF_MEMORY;
    }
...
pv_status_t pv_normalizer_tagger_tag_dot_number(pv_normalizer_token_t *token, pv_normalizer_token_list_t *token_list) {
    ...
    while (!is_first_token) {
        ...
        if (current_is_cardinal) {
            ...
            if ((current->tag == PV_NORMALIZER_TAG_PT_CARDINAL || current->tag == PV_NORMALIZER_TAG_PT_DIGITS) &&
                ((is_first_token && (strlen(string) <= 3)) || (!is_first_token && (strlen(string) == 3)))) {
                current_is_cardinal = true;
            } else if (is_first_token &&
                       (current->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL) &&
                       (strlen(string) <= 4)) {
                current_is_cardinal = true;
            } else if (is_first_token &&
                       (current->tag == PV_NORMALIZER_TAG_PT_CURRENCY)) {
                char character[PV_NORMALIZER_MAX_NUM_BYTES_PER_CHARACTER] = {0};
                int32_t num_bytes_character = 0;
                ...
                if ((strlen(string) - num_bytes_character) <= 3) {
                    current_is_cardinal = true;
                       ...
                       (current->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CURRENCY)) {
                char character[PV_NORMALIZER_MAX_NUM_BYTES_PER_CHARACTER] = {0};
                int32_t num_bytes_character = 0;
...
pv_status_t pv_normalizer_tagger_tag_number_range(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_NUMBER_RANGE_TO]) {
        const char *string = token->string;
        ...
        if (is_range && (num_hyphens == 1) && (num_cardinals == 2)) {
            ...
            token->next->tag = PV_NORMALIZER_TAG_PT_NUMBER_RANGE_TO;
        }
    }
    return PV_STATUS_SUCCESS;
}
...
void pv_normalizer_tagger_tag_decimal(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_DECIMAL_DIGITS]) {
        ...
        if (has_comma_before_current) {
            ...
            bool has_space_or_null_before_comma =
                    ...
                    (two_previous->tag == PV_NORMALIZER_TAG_PT_SPACE);
            bool has_cardinal_before_comma =
                    ((two_previous != NULL) &&
                     ((two_previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL) ||
                      (two_previous->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL)) &&
                     !two_previous->next_character_is_space);
            ...
            if (has_space_or_null_before_comma || has_cardinal_before_comma) {
                ...
                if (!found_non_digit) {
                    token->tag = PV_NORMALIZER_TAG_PT_DECIMAL_DIGITS;
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_NONE;
                    token->previous->tag = PV_NORMALIZER_TAG_PT_DECIMAL_POINT;
                }
            }
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_alphanum_spell_out(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_ALPHANUM_SPELL_OUT]) {
...
static pv_status_t pv_normalizer_tagger_tag_fraction(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_FRACTION_SLASH]) {
        const char *string = token->string;
        ...
        if (is_fraction && (num_slashes == 1) && (num_cardinals == 2)) {
            ...
            status = pv_normalizer_tagger_pt_tag_token(object, token, token_list);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                        pv_normalizer_tagger_pt_tag_token,
                        pv_status_to_string(status));
            ...
            status = pv_normalizer_tagger_pt_tag_token(object, token->next->next, token_list);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                        pv_normalizer_tagger_pt_tag_token,
                        pv_status_to_string(status));
            ...
            bool after_slash_is_number =
                    ((token->next->next->tag == PV_NORMALIZER_TAG_PT_CARDINAL) ||
                     (token->next->next->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL) ||
                     (token->next->next->tag == PV_NORMALIZER_TAG_PT_DECIMAL_DIGITS));

            if (before_slash_is_number && after_slash_is_number) {
                token->next->tag = PV_NORMALIZER_TAG_PT_FRACTION_SLASH;
            }
        }
    }
...
static pv_status_t pv_normalizer_tagger_tag_measurement(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_MEASUREMENT]) {
        const char *string = token->string;
        ...
        if ((previous != NULL) &&
            ((previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL) ||
             (previous->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL) ||
             (previous->tag == PV_NORMALIZER_TAG_PT_DECIMAL_DIGITS))) {
            ...
            if (trie_index >= 0) {
                token->tag = PV_NORMALIZER_TAG_PT_MEASUREMENT;
                token->tag_data_index = trie_index;
                if (PV_NORMALIZER_MEASUREMENTS_IS_FEMININE[trie_index] &&
                    previous->tag != PV_NORMALIZER_TAG_PT_DECIMAL_DIGITS) {
                    previous->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                }
                return PV_STATUS_SUCCESS;
        ...
        // measurement only eg. "ml"
        if ((previous == NULL || (previous != NULL && previous->next_character_is_space)) &&
            (token->next == NULL || strcmp(token->next->string, " ") == 0 || token->next_character_is_space)) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
        ...
        if ((previous != NULL) && (previous->tag == PV_NORMALIZER_TAG_PT_WORD)) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
            ...
            if (trie_index >= 0 && prev_trie_index >= 0) {
                ...
                previous->tag = PV_NORMALIZER_TAG_PT_MEASUREMENT;
                previous->tag_data_index = prev_trie_index;
                return PV_STATUS_SUCCESS;
            }
        } else if ((previous != NULL) && (previous->tag == PV_NORMALIZER_TAG_PT_DOT) &&
                   (two_previous != NULL) && (two_previous->tag == PV_NORMALIZER_TAG_PT_WORD)) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
            ...
            if (trie_index >= 0 && prev_trie_index >= 0) {
                ...
                two_previous->tag = PV_NORMALIZER_TAG_PT_MEASUREMENT;
        ...
        if (((previous != NULL) && (previous->tag == PV_NORMALIZER_TAG_PT_MEASUREMENT)) ||
            ((two_previous != NULL) && (two_previous->tag == PV_NORMALIZER_TAG_PT_MEASUREMENT))) {
            int32_t trie_index = -1;
            pv_status_t status = pv_normalizer_util_trie_search(object->measurement_trie, string, &trie_index);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
        ...
        if ((length > 1) && (isdigit(string[0]) || ((string[0] == '-') && isdigit(string[1])))) {
            ...
            if ((split_index) && (split_index < length)) {
                ...
                if (trie_index >= 0) {
                    ...
                    if ((token->tag == PV_NORMALIZER_TAG_PT_CARDINAL ||
                         token->tag == PV_NORMALIZER_TAG_PT_DIGITS) &&
                        ...
                        if (token->tag == PV_NORMALIZER_TAG_PT_DIGITS) {
                            token->tag = PV_NORMALIZER_TAG_PT_CARDINAL;
                        }
                    }
                    ...
                    if (token->tag == PV_NORMALIZER_TAG_PT_CARDINAL ||
                        token->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL ||
                        token->tag == PV_NORMALIZER_TAG_PT_DECIMAL_DIGITS) {
                        measurement_token->tag = PV_NORMALIZER_TAG_PT_MEASUREMENT;
        ...
        if (length > 2) {
            ...
            if ((split_index > 0) && (split_index < length)) {
                ...
                if (trie_index >= 0) {
                    ...
                    if (next_previous->tag == PV_NORMALIZER_TAG_PT_MEASUREMENT) {
                        ...
                        next->tag = PV_NORMALIZER_TAG_PT_PER_SLASH;

                        next->next->tag = PV_NORMALIZER_TAG_PT_MEASUREMENT;
                        ...
                        if (trie_index_two >= 0) {
                            ...
                            next_previous->tag = PV_NORMALIZER_TAG_PT_MEASUREMENT;
    ...
    // `miles/hour`
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_PER_SLASH]) {
        ...
        if ((strcmp(token->string, "/") == 0) && (previous != NULL) && (next != NULL)) {
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS_PT; i++) {
                if (strcmp(previous_upper_string, PV_NORMALIZER_MEASUREMENTS_PT[i]) == 0) {
                    measurement_before = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS_PT; i++) {
                if (strcmp(previous_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_PLURAL_PT[i]) == 0) {
                    measurement_before = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS_PT; i++) {
                if (strcmp(previous_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_SINGULAR_PT[i]) == 0) {
                    measurement_before = true;
                    break;
                }
            }
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS_PT; i++) {
                if (strcmp(next_upper_string, PV_NORMALIZER_MEASUREMENTS_PT[i]) == 0) {
                    measurement_after = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS_PT; i++) {
                if (strcmp(next_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_PLURAL_PT[i]) == 0) {
                    measurement_after = true;
                    break;
                }
            }
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_MEASUREMENTS_PT; i++) {
                if (strcmp(next_upper_string, PV_NORMALIZER_MEASUREMENTS_VERBALIZED_SINGULAR_PT[i]) == 0) {
                    measurement_after = true;
                    break;
                }
            }
            ...
            if (measurement_before && measurement_after) {
                token->tag = PV_NORMALIZER_TAG_PT_PER_SLASH;
            }
        }
    }
...
static pv_status_t pv_normalizer_tagger_pt_tag_time(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] >=
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_TIME_HOURS]) {
        return PV_STATUS_SUCCESS;
    }
    ...
    if (can_be_minutes) {
        ...
        bool has_hour_before_colon =
                (num_tokens_before > 1) &&
                (token->previous->previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL) &&
        ...
        if (has_colon_before_current && has_hour_before_colon) {
            ...
            if (smaller_than_61) {
                token->previous->previous->tag = PV_NORMALIZER_TAG_PT_TIME_HOURS;
                token->previous->previous->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                token->previous->tag = PV_NORMALIZER_TAG_PT_TIME_COLON;
                token->tag = PV_NORMALIZER_TAG_PT_TIME_MINUTES;
                token->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
    ...
    const char *string = token->string;
    int32_t length = (int32_t) strlen(string);
    if (length > 3 && string[length - 3] == 'h' && isdigit(string[0])) {
        int32_t split_index = 1;
        while ((split_index < length) && isdigit(string[split_index])) {
            split_index++;
        }
        ...
        status = pv_normalizer_tagger_pt_tag_token(object, hour_token, token_list);
        if (status != PV_STATUS_SUCCESS) {
            PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                    pv_normalizer_tagger_pt_tag_token,
                    pv_status_to_string(status));
        ...
        if (hour_token->tag == PV_NORMALIZER_TAG_PT_CARDINAL &&
            token->tag == PV_NORMALIZER_TAG_PT_CARDINAL) {
            ...
            if (smaller_than_25 && smaller_than_61) {
                hour_token->tag = PV_NORMALIZER_TAG_PT_TIME_HOURS;
                hour_token->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                h_token->tag = PV_NORMALIZER_TAG_PT_TIME_H;
                token->tag = PV_NORMALIZER_TAG_PT_TIME_MINUTES;
                token->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
            }
        }
...
pv_status_t pv_normalizer_tagger_tag_special_character(const pv_normalizer_tagger_pt_t *object, pv_normalizer_token_t *token) {
    PV_ASSERT(object);
    PV_ASSERT(token);

    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_SPECIAL_CHARACTER]) {
        const char *string = token->string;
        ...
        if (is_special_character && special_character_length == (int32_t) strlen(string)) {
            token->tag = PV_NORMALIZER_TAG_PT_SPECIAL_CHARACTER;
        }
    }
    return PV_STATUS_SUCCESS;
}
...
pv_status_t pv_normalizer_tagger_tag_url(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_DOT]) {
        const char *string = token->string;
        ...
        if ((strcmp(string, ".") == 0) &&
            ...
            token->tag = PV_NORMALIZER_TAG_PT_DOT;
        }
    }

    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_COLON]) {
        const char *string = token->string;
        ...
        if ((strcmp(string, ":") == 0) &&
            previous_is_non_space_character &&
            next_is_non_space_character) {
            token->tag = PV_NORMALIZER_TAG_PT_COLON;
        }
    }

    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_ACRONYM]) {
        ...
        for (int32_t i = 0; i < PV_NORMALIZER_NUM_ACRONYMS_PT; i++) {
            if (strcmp(upper_string, PV_NORMALIZER_ACRONYMS_PT[i]) == 0) {
                token->tag = PV_NORMALIZER_TAG_PT_ACRONYM;
                break;
            }
        }
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_TOP_LEVEL_DOMAIN]) {
        if ((token->previous != NULL) &&
            !token->previous->next_character_is_space &&
            (token->previous->tag == PV_NORMALIZER_TAG_PT_DOT)) {
            char *upper_string = NULL;
            pv_status_t status = pv_normalizer_util_upper(token->string, &upper_string);
            if (status != PV_STATUS_SUCCESS) {
                PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_TOP_LEVEL_DOMAINS_PT; i++) {
                if (strcmp(upper_string, PV_NORMALIZER_TOP_LEVEL_DOMAINS_PT[i]) == 0) {
                    token->tag = PV_NORMALIZER_TAG_PT_TOP_LEVEL_DOMAIN;
...
static pv_status_t pv_normalizer_tagger_tag_currency(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_CURRENCY]) {
        const char *string = token->string;
        int32_t length = (int32_t) strlen(string);
        ...
        for (int32_t i = 0; i < PV_NORMALIZER_NUM_CURRENCIES_PT; i++) {
            num_bytes_currency = (int32_t) strlen(PV_NORMALIZER_CURRENCIES_PT[i]);
            if (strncmp(string, PV_NORMALIZER_CURRENCIES_PT[i], num_bytes_currency) == 0) {
                currency_index = i;
        ...
        if (starts_with_currency && (num_bytes_currency != length)) {
            ...
            if (only_digits) {
                token->tag = PV_NORMALIZER_TAG_PT_CURRENCY;
                token->tag_data_index = currency_index;
                if (PV_NORMALIZER_CURRENCIES_IS_FEMININE[currency_index]) {
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                } else {
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
                }
        ...
        if (isdigit(string[0])) {
            ...
            while (i < length) {
                ...
                if (!isdigit(string[i])) {
                    for (int32_t j = 0; j < PV_NORMALIZER_NUM_CURRENCIES_PT; j++) {
                        num_bytes_currency = (int32_t) strlen(PV_NORMALIZER_CURRENCIES_PT[j]);
                        if (strncmp(string + i, PV_NORMALIZER_CURRENCIES_PT[j], num_bytes_currency) == 0) {
            ...
            if (only_digits && ends_with_currency) {
                ...
                bool invalid_digits =
                        (token->previous != NULL) &&
                        (strcmp(token->previous->string, ",") == 0) &&
                        (length_digits != 2) && (length_digits != 1);
                ...
                if ((token->previous != NULL) && token->previous->previous != NULL) {
                    ...
                    if (is_currency_decimal) {
                        pv_normalizer_token_tag_pt_t two_previous_tag = two_previous->tag;
                        ...
                        if (two_previous_tag == PV_NORMALIZER_TAG_PT_CARDINAL) {
                            merged_token->tag = PV_NORMALIZER_TAG_PT_CURRENCY;
                        } else {
                            merged_token->tag = PV_NORMALIZER_TAG_PT_NEGATIVE_CURRENCY;
                        }
        ...
        if (only_digits && (token->previous != NULL) && (token->previous->previous != NULL)) {
            ...
            bool is_currency_decimal =
                    (strcmp(previous->string, ",") == 0) &&
                    (!previous->next_character_is_space) &&
                    ((two_previous->tag == PV_NORMALIZER_TAG_PT_CURRENCY) ||
                     (two_previous->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CURRENCY)) &&
                    (!two_previous->next_character_is_space);
        ...
        if (string[0] == '-') {
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_CURRENCIES_PT; i++) {
                num_bytes_currency = (int32_t) strlen(PV_NORMALIZER_CURRENCIES_PT[i]);
                if (strncmp(string + 1, PV_NORMALIZER_CURRENCIES_PT[i], num_bytes_currency) == 0) {
                    currency_index = i;
            ...
            if (second_character_is_currency) {
                ...
                if (only_digits) {
                    token->tag = PV_NORMALIZER_TAG_PT_NEGATIVE_CURRENCY;
                    token->tag_data_index = currency_index;
                    if (PV_NORMALIZER_CURRENCIES_IS_FEMININE[currency_index]) {
                        token->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                    } else {
        ...
        for (int32_t i = 0; i < PV_NORMALIZER_NUM_CURRENCIES_PT; i++) {
            if (strcmp(string, PV_NORMALIZER_CURRENCIES_PT[i]) == 0) {
                currency_index = i;
                is_currency = true;
                break;
            }
        ...
        if (is_currency && (previous_is_cardinal || previous_is_decimal)) {
            token->tag = PV_NORMALIZER_TAG_PT_CURRENCY_SYMBOL;
            token->tag_data_index = currency_index;
            if (PV_NORMALIZER_CURRENCIES_IS_FEMININE[currency_index] && previous_is_cardinal) {
                previous->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
            } else {
                previous->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
            }
...
pv_status_t pv_normalizer_tagger_tag_abbreviation(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_ABBREVIATION]) {
        ...
        if (is_dot) {
            ...
            while (start_token &&
                   (start_token->tag == PV_NORMALIZER_TAG_PT_SPECIAL_CHARACTER ||
                    start_token->tag == PV_NORMALIZER_TAG_PT_SINGLE_QUOTE) &&
            ...
            for (int32_t i = 0; i < PV_NORMALIZER_NUM_ABBREVIATIONS_PT; i++) {
                if (strcmp(potential_abbreviation, PV_NORMALIZER_ABBREVIATIONS_PT[i]) == 0) {
                    token->tag = PV_NORMALIZER_TAG_PT_ABBREVIATION;
                    token->tag_data_index = i;
                    break;
                }
            }
...
pv_status_t pv_normalizer_tagger_tag_digits_sequence(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_DIGITS_WITH_PARENTHESES]) {
        ...
        if ((length == 5) && (string[0] == '(') && (string[4] == ')')) {
            if (isdigit(string[1]) && isdigit(string[2]) && isdigit(string[3])) {
                token->tag = PV_NORMALIZER_TAG_PT_DIGITS_WITH_PARENTHESES;
            }
        }
    }

    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_DIGITS]) {
        ...
        if (is_digits_with_dashes) {
            ...
            while (current != NULL) {
                ...
                if ((strlen(current->string) == 1) && pv_normalizer_tagger_is_digit_separator(current->string[0])) {
                    current->tag = PV_NORMALIZER_TAG_PT_DIGITS_SEPARATOR;
                } else {
                    current->tag = PV_NORMALIZER_TAG_PT_DIGITS;
                }
        ...
        bool is_second_number_in_digits_sequence =
                ...
                (previous_skip_space->tag == PV_NORMALIZER_TAG_PT_DIGITS_WITH_PARENTHESES);
        pv_normalizer_token_t *two_previous = pv_normalizer_token_get_nth_token_before(token, 2, false);
        ...
        bool is_third_number_in_digits_sequence =
                (four_previous != NULL) &&
                (four_previous->tag == PV_NORMALIZER_TAG_PT_DIGITS_WITH_PARENTHESES) &&
                (two_previous->tag == PV_NORMALIZER_TAG_PT_DIGITS);
        ...
        if (is_digits_with_dots) {
            pv_normalizer_token_t *current = token;
            while (current != NULL) {
                if ((current->tag == PV_NORMALIZER_TAG_PT_CARDINAL) ||
...
int32_t pv_normalizer_tagger_get_month_index(const char *string) {
    ...
    for (int32_t i = 0; i < PV_NORMALIZER_NUM_MONTH_NAMES_PT; i++) {
        if (strcmp(string, PV_NORMALIZER_MONTH_NAMES_PT[i]) == 0) {
            return i;
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_date(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_DATE_MONTH]) {
        const char *string = token->string;
        int32_t length = (int32_t) strlen(string);
        ...
        if (index_month_name >= 0) {
            token->tag = PV_NORMALIZER_TAG_PT_DATE_MONTH;
            token->tag_data_index = index_month_name;

            pv_normalizer_token_t *previous_skip_space = pv_normalizer_token_get_nth_token_before(token, 1, true);
            bool is_previous_day =
                    (previous_skip_space != NULL) &&
                    previous_skip_space->tag == PV_NORMALIZER_TAG_PT_CARDINAL &&
            ...
            if (is_previous_day) {
                previous_skip_space->tag = PV_NORMALIZER_TAG_PT_DATE_DAY;
            }
            return PV_STATUS_SUCCESS;
        }
        ...
        if (current_is_year) {
            ...
            if (is_date) {
                token->tag = PV_NORMALIZER_TAG_PT_DATE_YEAR;
                previous->tag = PV_NORMALIZER_TAG_PT_DATE_SEPARATOR;
                three_previous->tag = PV_NORMALIZER_TAG_PT_DATE_SEPARATOR;

                if (two_previous_is_month && four_previous_is_day) {
                    two_previous->tag = PV_NORMALIZER_TAG_PT_DATE_MONTH;
                    four_previous->tag = PV_NORMALIZER_TAG_PT_DATE_DAY;
                } else {
                    two_previous->tag = PV_NORMALIZER_TAG_PT_DATE_DAY;
                    four_previous->tag = PV_NORMALIZER_TAG_PT_DATE_MONTH;
                }
        ...
        if (four_previous_is_year) {
            ...
            bool is_date =
                    ...
                    current_is_day;
            if (is_date) {
                four_previous->tag = PV_NORMALIZER_TAG_PT_DATE_YEAR;
                three_previous->tag = PV_NORMALIZER_TAG_PT_DATE_SEPARATOR;
                two_previous->tag = PV_NORMALIZER_TAG_PT_DATE_MONTH;
                previous->tag = PV_NORMALIZER_TAG_PT_DATE_SEPARATOR;
                token->tag = PV_NORMALIZER_TAG_PT_DATE_DAY;
        ...
        current_is_year =
                ...
                (strlen(string) == 4) &&
                previous_skip_space != NULL &&
                previous_skip_space->tag == PV_NORMALIZER_TAG_PT_DATE_MONTH;
        if (current_is_year) {
            token->tag = PV_NORMALIZER_TAG_PT_DATE_YEAR;
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_name(
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_NAME_INITIAL_LETTER]) {
        const char *string = token->string;
        ...
        if (is_dot) {
            ...
            if (previous_token != NULL) {
                ...
                if (is_capitalized && (strlen(previous_token->string) == 1)) {
                    previous_token->tag = PV_NORMALIZER_TAG_PT_NAME_INITIAL_LETTER;
                    token->tag = PV_NORMALIZER_TAG_PT_NAME_INITIAL_DOT;
                }
            }
        }
    }
...
bool pv_normalizer_is_ordinal_too_large(const pv_normalizer_token_t *token) {
    PV_ASSERT(token);

    const char *string = token->string;
    if (string[0] == '-') {
        return pv_normalizer_util_string_number_greater_than_int(string + 1, MAX_VERBALIZED_CARDINAL_PT);
    }
    return pv_normalizer_util_string_number_greater_than_int(string, MAX_VERBALIZED_CARDINAL_PT);
}
...
pv_status_t pv_normalizer_tagger_tag_ordinal(pv_normalizer_token_t *token, pv_normalizer_token_list_t *token_list) {
    ...
    int32_t length = (int32_t) strlen(string);
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_ORDINAL]) {

        // superscript case: 1º/1ª, º and ª are two bytes
        if (length >= 3) {
            bool found_non_digit = false;
            int32_t number_end_index = 0;
            int32_t i = 0;
            while (i < length) {
                if (!(isdigit(string[i]))) {
                    found_non_digit = true;
            ...
            if (!found_non_digit || (number_end_index != (length - 2))) {
                return PV_STATUS_SUCCESS;
            }

            char *superscript_string = calloc(2 + 1, sizeof(char));
            if (!superscript_string) {
                PV_ERROR_REPORT(
                        &pv_error_msg_alloc,
                        PV_ERROR_ARGS_PUBLIC_EMPTY(),
            ...
            strncpy(superscript_string, string + number_end_index, 2);

            int32_t superscript_index = -1;
            for (int32_t j = 0; j < PV_NORMALIZER_NUM_VALID_SUPERSCRIPT_PT; j++) {
                if (strcmp(superscript_string, ORDINAL_VALID_SUPERSCRIPT_PT[j]) == 0) {
                    superscript_index = j;
                    break;
                }
            }
            free(superscript_string);

            if (superscript_index == -1) {
                return PV_STATUS_SUCCESS;
            }

            bool is_feminine = ORDINAL_SUPERSCRIPT_IS_FEMININE_PT[superscript_index];

            char *number_string = calloc(number_end_index + 1, sizeof(char));
            if (!number_string) {
                PV_ERROR_REPORT(
                        &pv_error_msg_alloc,
                        PV_ERROR_ARGS_PUBLIC_EMPTY(),
            ...
            strncpy(number_string, string, number_end_index);

            if (!pv_normalizer_util_string_number_greater_than_int(number_string, MAX_VERBALIZED_CARDINAL_PT)) {
                token->tag = PV_NORMALIZER_TAG_PT_ORDINAL;
                if (is_feminine) {
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                } else {
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
    ...
    bool is_a = (strcmp(string, "a") == 0);
    bool is_o = (strcmp(string, "o") == 0);
    if (is_a || is_o) {
        pv_normalizer_token_t *previous = pv_normalizer_token_get_nth_token_before(token, 1, false);
        pv_normalizer_token_t *two_previous = pv_normalizer_token_get_nth_token_before(token, 2, false);
        bool is_before_apostrophe = token->next != NULL && strcmp(token->next->string, APOSTROPHE) == 0;
        if (previous != NULL && two_previous != NULL &&
            (token->next_character_is_space || token->next == NULL || is_before_apostrophe) &&
            (strcmp(previous->string, ".") == 0) &&
            (two_previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL ||
             two_previous->tag == PV_NORMALIZER_TAG_PT_NEGATIVE_CARDINAL)) {

            // two_prev could be final 3 digits of comma number eg. `1.333.a`
            if (two_previous->tag == PV_NORMALIZER_TAG_PT_CARDINAL && (strlen(two_previous->string) == 3)) {
                pv_status_t status = pv_normalizer_tagger_tag_dot_number(two_previous, token_list);
                if (status != PV_STATUS_SUCCESS) {
                    PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                            pv_normalizer_tagger_tag_dot_number,
                            pv_status_to_string(status));
            ...
            if (pv_normalizer_is_ordinal_too_large(two_previous)) {
                token->tag = PV_NORMALIZER_TAG_PT_LETTER_SPELL_OUT;
                return PV_STATUS_SUCCESS;
            }

            pv_normalizer_token_tag_pt_t tag = two_previous->tag;
            ...
            if (tag == PV_NORMALIZER_TAG_PT_CARDINAL) {
                merged_token->tag = PV_NORMALIZER_TAG_PT_ORDINAL;
            } else {
                merged_token->tag = PV_NORMALIZER_TAG_PT_NEGATIVE_ORDINAL;
            }
            if (is_a) {
                merged_token->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
            } else {
                merged_token->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
            }
        }
    }
...
pv_status_t pv_normalizer_tagger_tag_negative_ordinal(pv_normalizer_token_t *token) {
    ...
    if (PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[token->tag] <
        PV_NORMALIZER_TOKEN_TAG_PT_WEIGHTS[PV_NORMALIZER_TAG_PT_NEGATIVE_ORDINAL]) {
        char *string = token->string;
        int32_t length = (int32_t) strlen(string);
        ...
        if (length >= 4 && string[0] == '-') {
            ...
            char *number_string = calloc(number_end_index, sizeof(char));
            if (!number_string) {
                PV_ERROR_REPORT(
                        &pv_error_msg_alloc,
                        PV_ERROR_ARGS_PUBLIC_EMPTY(),
            ...
            strncpy(number_string, string + 1, number_end_index - 1);

            if (!pv_normalizer_util_string_number_greater_than_int(number_string, MAX_VERBALIZED_CARDINAL_PT)) {
                token->tag = PV_NORMALIZER_TAG_PT_NEGATIVE_ORDINAL;
                if (is_feminine) {
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_FEMININE;
                } else {
                    token->gender = PV_NORMALIZER_TOKEN_GENDER_MASCULINE;
...
pv_status_t pv_normalizer_tagger_pt_merge_word_token(pv_normalizer_token_list_t *token_list) {
    PV_ASSERT(token_list);

    pv_normalizer_token_t *current = token_list->head;
    while (current) {
        if (current->tag == PV_NORMALIZER_TAG_PT_WORD || current->tag == PV_NORMALIZER_TAG_PT_SINGLE_QUOTE) {
            ...
            while ((last->next) && (!last->next_character_is_space) && (!last->next_character_is_hyphen)) {
                if ((last->next->tag == PV_NORMALIZER_TAG_PT_WORD || last->next->tag == PV_NORMALIZER_TAG_PT_SINGLE_QUOTE) ||
                    (last->tag == PV_NORMALIZER_TAG_PT_SINGLE_QUOTE && last->next->tag == PV_NORMALIZER_TAG_PT_DATE_MONTH)) {
                    last = last->next;
            ...
            if (first != last) {
                bool merge_original = (last->tag == PV_NORMALIZER_TAG_PT_SINGLE_QUOTE &&
                                       (last->next_character_is_space ||
                                        last->next == NULL ||
                                        (last->next && last->next->tag == PV_NORMALIZER_TAG_PT_SPACE)));
                ...
                merged_token->tag = PV_NORMALIZER_TAG_PT_WORD;
