...
struct pv_orca_prior_encoder_film_generator {
...
pv_status_t PV_MOCKABLE(pv_orca_prior_encoder_film_generator_param_serialize)(
    ...
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_embed_param_serialize,
                pv_status_to_string(status));
        return status;
    }

    for (int32_t i = 0; i < param->num_blocks; i++) {
        status = pv_rope_transformer_param_serialize(
                ypu,
...
pv_status_t PV_MOCKABLE(pv_orca_prior_encoder_film_generator_param_load)(
    ...
    *param = NULL;
    ...
    pv_orca_prior_encoder_film_generator_param_t *p = pv_ypu_host_alloc(
            ypu,
            sizeof(pv_orca_prior_encoder_film_generator_param_t));
    if (!p) {
        PV_ERROR_REPORT(
    ...
    memset(p, 0, sizeof(pv_orca_prior_encoder_film_generator_param_t));

    size_t count = pv_fread(&(p->num_blocks), sizeof(int32_t), 1, f);
    if (count != 1) {
        pv_orca_prior_encoder_film_generator_param_delete(ypu, p);
        return PV_STATUS_IO_ERROR;
    }
    if (p->num_blocks <= 0) {
        pv_orca_prior_encoder_film_generator_param_delete(ypu, p);
    ...
    pv_status_t status = pv_embed_param_load(
            ypu,
            f,
            (pv_embed_param_t **) &(p->embed_param));
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_embed_param_load,
                pv_status_to_string(status));
    ...
    for (int32_t i = 0; i < p->num_blocks; i++) {
        status = pv_rope_transformer_param_load(
                ypu,
                f,
...
void PV_MOCKABLE(pv_orca_prior_encoder_film_generator_param_delete)(
    ...
    if (param) {
        if (param->transformers_param) {
            ...
            pv_ypu_host_free(ypu, (pv_rope_transformer_param_t **) (param->transformers_param));
        }
...
pv_status_t PV_MOCKABLE(pv_orca_prior_encoder_film_generator_init)(
    ...
    *object = NULL;
    ...
    pv_orca_prior_encoder_film_generator_t *o = pv_ypu_host_alloc(
            ypu,
            sizeof(pv_orca_prior_encoder_film_generator_t));
    if (!o) {
        PV_ERROR_REPORT(
                &pv_error_msg_ypu_host_alloc,
                PV_ERROR_ARGS_PUBLIC_EMPTY(),
    ...
    memset(o, 0, sizeof(pv_orca_prior_encoder_film_generator_t));
    ...
    if (status != PV_STATUS_SUCCESS) {
        PV_ERROR_REPORT_MODULE_FUNCTION_STATUS_INTERNAL_HELPER(
                pv_embed_init,
                pv_status_to_string(status));
        pv_orca_prior_encoder_film_generator_delete(ypu, o);
        return status;
    }
