...
pv_status_t PV_MOCKABLE(pv_vocos_backbone_param_serialize)(const pv_vocos_backbone_param_t *param, FILE *file) {
    ...
    pv_status_t status = pv_layer_norm_param_serialize(param->layer_norm_pre_param, file);
    PV_CHECK_STATUS(status);

    status = pv_layer_norm_param_serialize(param->layer_norm_post_param, file);
    PV_CHECK_STATUS(status);
    ...
    for (int32_t i = 0; i < param->num_convnext_layers; i++) {
        status = pv_convnext_param_serialize(param->convnext_layers_param[i], file);
...
pv_status_t PV_MOCKABLE(pv_vocos_backbone_param_load)(FILE *f, pv_vocos_backbone_param_t **param) {
    ...
    *param = NULL;
    ...
    pv_vocos_backbone_param_t *p = calloc(1, sizeof(pv_vocos_backbone_param_t));
    PV_CHECK_ALLOC(p);

    pv_status_t status = pv_layer_norm_param_load(f, (pv_layer_norm_param_t **) &(p->layer_norm_pre_param));
    if (status != PV_STATUS_SUCCESS) {
        pv_vocos_backbone_param_delete(p);
        return status;
    }

    status = pv_layer_norm_param_load(f, (pv_layer_norm_param_t **) &(p->layer_norm_post_param));
    if (status != PV_STATUS_SUCCESS) {
        pv_vocos_backbone_param_delete(p);
        return status;
    }
    ...
    p->convnext_layers_param = calloc(p->num_convnext_layers, sizeof(pv_convnext_param_t *));
    ...
    for (int32_t i = 0; i < p->num_convnext_layers; i++) {
        status = pv_convnext_param_load(f, (pv_convnext_param_t **) &(p->convnext_layers_param[i]));
        if (status != PV_STATUS_SUCCESS) {
            pv_vocos_backbone_param_delete(p);
            return status;
        }
    ...
    if (param) {
        if (param->convnext_layers_param) {
            for (int32_t i = param->num_convnext_layers - 1; i >= 0; i--) {
                pv_convnext_param_delete((pv_convnext_param_t *) param->convnext_layers_param[i]);
            }

            free((pv_convnext_param_t **) (param->convnext_layers_param));
        }

        pv_layer_norm_param_delete((pv_layer_norm_param_t *) (param->layer_norm_post_param));

        pv_layer_norm_param_delete((pv_layer_norm_param_t *) (param->layer_norm_pre_param));
...
pv_status_t PV_MOCKABLE(pv_vocos_backbone_init)(
    ...
    *object = NULL;
    ...
    pv_vocos_backbone_t *o = calloc(1, sizeof(pv_vocos_backbone_t));
    if (!o) {
        return PV_STATUS_OUT_OF_MEMORY;
    }
    ...
    pv_status_t status = pv_layer_norm_init(param->layer_norm_pre_param, &(o->layer_norm_pre));
    if (status != PV_STATUS_SUCCESS) {
        pv_vocos_backbone_delete(o);
        return status;
    }

    status = pv_layer_norm_init(param->layer_norm_post_param, &(o->layer_norm_post));
    if (status != PV_STATUS_SUCCESS) {
        pv_vocos_backbone_delete(o);
        return status;
    }

    o->convnext_layers = calloc(param->num_convnext_layers, sizeof(pv_convnext_t *));
    if (!(o->convnext_layers)) {
        pv_vocos_backbone_delete(o);
        return PV_STATUS_OUT_OF_MEMORY;
    }

    int32_t buffer_convnext_1_channels = param->convnext_layers_param[0]->conv_depthwise_param->num_channels;
    status = pv_buffer_init(buffer_convnext_1_channels, &(o->buffer_vocos_backbone_convnext_1));
    if (status != PV_STATUS_SUCCESS) {
        pv_vocos_backbone_delete(o);
        return status;
    }

    int32_t buffer_convnext_2_channels = param->convnext_layers_param[0]->conv_1_param->output_channels;
    status = pv_buffer_q510_init(buffer_convnext_2_channels, &(o->buffer_vocos_backbone_convnext_2));
    if (status != PV_STATUS_SUCCESS) {
        pv_vocos_backbone_delete(o);
        return status;
    }
    ...
    for (int32_t i = 0; i < param->num_convnext_layers; i++) {
        status = pv_convnext_init(
                ...
                o->buffer_vocos_backbone_convnext_1,
                o->buffer_vocos_backbone_convnext_2,
                &(o->convnext_layers[i]));
        if (status != PV_STATUS_SUCCESS) {
            pv_vocos_backbone_delete(o);
            return status;
...
void PV_MOCKABLE(pv_vocos_backbone_delete)(pv_vocos_backbone_t *object) {
    if (object) {
        pv_buffer_q510_delete(object->buffer_vocos_backbone_convnext_2);
        pv_buffer_delete(object->buffer_vocos_backbone_convnext_1);

        if (object->convnext_layers) {
            for (int32_t i = object->param->num_convnext_layers - 1; i >= 0; i--) {
                pv_convnext_delete(object->convnext_layers[i]);
            }
            free(object->convnext_layers);
        }

        pv_layer_norm_delete(object->layer_norm_post);
        pv_layer_norm_delete(object->layer_norm_pre);
...
pv_status_t PV_MOCKABLE(pv_vocos_backbone_forward)(pv_vocos_backbone_t *object, int32_t n, const float *x, float *y) {
    ...
    pv_status_t status = pv_layer_norm_forward(object->layer_norm_pre, n, x, y);
    if (status != PV_STATUS_SUCCESS) {
        return status;
    }
    ...
    pv_buffer_q510_free(object->buffer_vocos_backbone_convnext_2);
    pv_buffer_free(object->buffer_vocos_backbone_convnext_1);

    status = pv_layer_norm_forward(object->layer_norm_post, n, y, y);
    if (status != PV_STATUS_SUCCESS) {
        return status;
    }
