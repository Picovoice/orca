...
static pv_normalizer_use_cases_fr_t ALL_TEST_USE_CASES[] = {
...
static const char TAGGER_TEST_SENTENCE[] = "je prends de la vitamine b-12, c, et 15 autres!";

static const pv_normalizer_token_tag_fr_t TAGGER_TEST_SENTENCE_TAGS[] = {
...
static const char NOUN_GENDER_DICT_PATH[] = "orca/test_data/noun_gender_dict/noun_gender_dict_fr.txt";
...
static void test_pv_normalizer_tagger_tokenize_tag_and_check_tags_helper(
        ...
        bool preserve_word_boundary,
        const pv_normalizer_token_tag_fr_t *target_tags) {
...
static void test_pv_normalizer_tagger_tag_word_preserve_boundary(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_WORD_NORMALIZER_FR};

    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
...
static void test_pv_normalizer_tagger_tag_punctuation(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_PUNCTUATION_NORMALIZER_FR};
...
static void test_pv_normalizer_tagger_tag_cardinal(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_CARDINAL_NORMALIZER_FR};
...
static void test_pv_normalizer_tagger_tag_negative_cardinal(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_NEGATIVE_CARDINAL_NORMALIZER_FR};
...
static void test_pv_normalizer_tagger_tag_custom_pronunciation(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_CUSTOM_PRONUNCIATION_NORMALIZER_FR};
...
static void test_pv_normalizer_tagger_tag_custom_pronunciation_all_phonemes(void) {
    ...
    const char sentence[] = "{salut|a ã b d e f g i j k l m n o õ p s t u v w y z ø ŋ œ œ̃ ɔ ə ɛ ɛ̃ ɥ ɲ ʁ ʃ ʒ}";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_CUSTOM_PRONUNCIATION};
...
static void test_pv_normalizer_tagger_tag_invalid_use_case(void) {
    static pv_normalizer_use_cases_fr_t invalid_use_cases[] = {99};
...
static void test_pv_normalizer_tagger_tag_number_range(void) {
    const char sentence[] = "12-5 -4-2 salut 43- -9 5-a a-5 6-33 50-50-50 1--3 123–12";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_CARDINAL,
...
static void test_pv_normalizer_tagger_tag_hyphenated_string(void) {
    ...
    const char sentence[] = "4-minute salut -563 quelque-part 2-2 salut- some–where";
...
static void test_pv_normalizer_tagger_tag_ordinal(void) {
    const char *sentence =
            "salut 1er 2e 3e 6e 7e 8e 9e 10e 11ème 12e 13e 21e 52e 143e 2000000000000e 10000000000000000000e";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
            PV_NORMALIZER_TAG_FR_SPACE,
...
static void test_pv_normalizer_tagger_tag_invalid_ordinal(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_ORDINAL_NORMALIZER_FR};

    const char *sentences[] = {"1re", "1ee", "1r", "4s", "11t", "12u", "13v", "35x"};
    ...
    for (int32_t i = 0; i < PV_ARRAY_LEN(sentences); i++) {
        ...
        pv_test_true(status == PV_STATUS_SUCCESS, "failed to tag sentence: `%s`", sentence);
        pv_test_true(
                token_list->head->tag != PV_NORMALIZER_TAG_FR_ORDINAL,
...
static void test_pv_normalizer_tagger_tag_negative_ordinal(void) {
    const char sentence[] =
            "salut -1er -2e -3e -6e -10e -11e -12e -13e -21e -52e -143e -2000000000000e -10000000000000000000e  −1er";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
...
static void test_pv_normalizer_tagger_tag_invalid_negative_ordinal(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {
            PV_NORMALIZER_USE_ORDINAL_NORMALIZER_FR,
            PV_NORMALIZER_USE_NEGATIVE_ORDINAL_NORMALIZER_FR
    };

    const char *sentences[] = {"-1re", "-1ee", "-1r", "-4s", "-11t", "-12u", "-13v", "-35x", "-2s3e", "-er"};
    ...
    for (int32_t i = 0; i < PV_ARRAY_LEN(sentences); i++) {
        ...
        pv_test_true(status == PV_STATUS_SUCCESS, "failed to tag sentence: `%s`", sentence);
        pv_test_true(
                token_list->head->tag != PV_NORMALIZER_TAG_FR_NEGATIVE_ORDINAL,
...
static void test_pv_normalizer_tagger_correct_num_tag_weights(void) {
    pv_test_true(
            PV_ARRAY_LEN(PV_NORMALIZER_TOKEN_TAG_FR_WEIGHTS) == PV_NORMALIZER_TAG_FR_NUM_TAGS,
            "Number of tags and tag weights do not match. Have %d tags and %d weights",
            PV_ARRAY_LEN(PV_NORMALIZER_TOKEN_TAG_FR_WEIGHTS),
            PV_NORMALIZER_TAG_FR_NUM_TAGS);
}

static void test_pv_normalizer_tagger_tag_special_characters(void) {
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_SPECIAL_CHARACTER_NORMALIZER_FR};

    const char sentence[] = "salut 5% @@ °C RPM 90 CM²";
    const char *sentence_strings[] = {"salut", "5", "%", "@", "@", "°C", "RPM", "90", "CM²"};
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_NONE,
            PV_NORMALIZER_TAG_FR_NONE,
...
static void test_pv_normalizer_tagger_tag_empty_custom_pron(void) {
    ...
    pv_test_true(current != NULL, "token is empty");
    pv_test_true(
            current->tag == PV_NORMALIZER_TAG_FR_CUSTOM_PRONUNCIATION,
            "incorrect tag for `%s`: got `%d`, expected `%d`",
            current->string,
            current->tag,
            PV_NORMALIZER_TAG_FR_CUSTOM_PRONUNCIATION);
...
static void test_pv_normalizer_tagger_tag_weights_order(void) {
    pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_WORD_NORMALIZER_FR};
    ...
    status = pv_normalizer_token_init(0, 4, "{|AY}", false, true, false, &empty_token);
    pv_test_true(status == PV_STATUS_SUCCESS, "failed to initialize token");
    empty_token->tag = PV_NORMALIZER_TAG_FR_WORD;
    ...
    pv_test_true(current != NULL, "token is empty");
    pv_test_true(
            current->tag == PV_NORMALIZER_TAG_FR_WORD,
            "incorrect tag for `%s`: got `%d`, expected `%d`",
            current->string,
            current->tag,
            PV_NORMALIZER_TAG_FR_WORD);
    ...
    pv_normalizer_use_cases_fr_t use_cases2[] = {
            PV_NORMALIZER_USE_WORD_NORMALIZER_FR,
            PV_NORMALIZER_USE_CUSTOM_PRONUNCIATION_NORMALIZER_FR};
...
static void test_pv_normalizer_tagger_next_char_is_space(void) {
    ...
    const char sentence[] = "4-minute, salut -563 quelque-part 2-2 salut-";
...
static void test_pv_normalizer_tagger_tag_decimal(void) {
    ...
    const pv_normalizer_token_tag_fr_t sentence_tags2[] = {
            PV_NORMALIZER_TAG_FR_PUNCTUATION,
            PV_NORMALIZER_TAG_FR_SPACE,
            PV_NORMALIZER_TAG_FR_CARDINAL};
...
static void test_pv_normalizer_tagger_tag_alphanum_spell_out(void) {
    const char sentence[] = "HTML5 A87B 12A 12 ÇL1";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_LETTER_SPELL_OUT,
            PV_NORMALIZER_TAG_FR_LETTER_SPELL_OUT,
            PV_NORMALIZER_TAG_FR_LETTER_SPELL_OUT,
...
static void test_pv_normalizer_tagger_tag_measurement(void) {
    const char sentence[] = "5 g 1ml -7,3l salut ml 10,1km 5°C 1.500km -1.000.111g 25ft lb contexte clair ml g";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_CARDINAL,
            PV_NORMALIZER_TAG_FR_SPACE,
...
static void test_pv_normalizer_tagger_tag_per_measurement(void) {
    const char sentence[] = "5 km/m 10 oz/km 1,1m/l -5,41kg/ft m/s 7 km/h 100km/h 1.000.111m/s";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_CARDINAL,
            PV_NORMALIZER_TAG_FR_SPACE,
            PV_NORMALIZER_TAG_FR_MEASUREMENT,
...
static void test_pv_normalizer_tagger_tag_dot_ordinal(void) {
    const char *sentence = "1.005e 11.005e 111.005e -5.123.123.123e 1111.000e 1.1.000e 1. 000e";
...
static void test_pv_normalizer_tagger_tag_url(void) {
    const char *sentence = "www.example.com https://salut.xyz/";
...
static void test_pv_normalizer_tagger_tag_abbreviations(void) {
    const char *sentence = "e.g. cette i.e. M. Ave. Rd. Dr. Ms. Gov.";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
...
static void test_pv_normalizer_tagger_tag_date(void) {
    ...
    const char *sentence_two = "3 juin 2020 8 sept 1991 13-juin-2024 32-10-2024 1 déc";
    const pv_normalizer_token_tag_fr_t sentence_tags_two[] = {
            PV_NORMALIZER_TAG_FR_DATE_DAY,
            PV_NORMALIZER_TAG_FR_SPACE,
            PV_NORMALIZER_TAG_FR_DATE_MONTH,
            PV_NORMALIZER_TAG_FR_SPACE,
...
static void test_pv_normalizer_tagger_tag_name(void) {
    const char *sentence = "C'est J. R. R. Tolkien John F. Kennedy 8:00";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
            PV_NORMALIZER_TAG_FR_SPACE,
            PV_NORMALIZER_TAG_FR_LETTER_SPELL_OUT,
...
static void test_pv_normalizer_tagger_tag_single_quote(void) {
    const char *sentence = "Elle a demandé, 'comment vas-tu.' 'Bonjour@' 'h -1er'";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
            PV_NORMALIZER_TAG_FR_WORD,
            PV_NORMALIZER_TAG_FR_WORD,
...
static void test_pv_normalizer_tagger_length_future_past_context_after_tag(void) {
    const char *sentence = "dans 3-5 jours. #AB12 www.exemple.com 9:30 1/2 1/1.000 1.123e 2ml 2m/h -1.900.876 $10.000,00 Dr.A.B Dr. e.g.";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
            PV_NORMALIZER_TAG_FR_SPACE,
            PV_NORMALIZER_TAG_FR_CARDINAL,
...
static void test_pv_normalizer_tagger_length_future_past_context_after_tag_date_digits(void) {
    const char *sentence = "12/25/2023 03-Juin-1829 (778) 239-1823 102-291-4920 123.456.789";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_DATE_MONTH,
            PV_NORMALIZER_TAG_FR_DATE_SEPARATOR,
            PV_NORMALIZER_TAG_FR_DATE_DAY,
...
static void test_pv_normalizer_tagger_length_future_past_context_single_quote(void) {
    const char *sentence = "Je recommande 'Le Petit Prince' !";
    const pv_normalizer_token_tag_fr_t sentence_tags[] = {
            PV_NORMALIZER_TAG_FR_WORD,
            PV_NORMALIZER_TAG_FR_SPACE,
            PV_NORMALIZER_TAG_FR_WORD,
    ...
    const int32_t sentence_futures[11] = {0};
    const int32_t sentence_pasts[11] = {0};
...
static void test_pv_normalizer_tagger_init_failure(void) {
    PV_SET_MOCK_RETURN_VAL(pv_normalizer_tagger_fr_init, PV_STATUS_OUT_OF_MEMORY);
    ...
    pv_test_error_message(
            ...
            "`pv_normalizer_tagger_fr_init` failed with status `OUT_OF_MEMORY`.",
            true,
...
static void test_pv_normalizer_tagger_tag_currency_helper_failure(void) {
    ...
    static pv_normalizer_use_cases_fr_t use_cases[] = {PV_NORMALIZER_USE_CURRENCY_NORMALIZER_FR};
    pv_normalizer_tagger_t *tagger = NULL;
    pv_normalizer_token_list_t *token_list = NULL;
...
const pv_test_suite_t PV_NORMALIZER_TAGGER_FR_TEST_SUITE = {
        .name = "normalizer_tagger_fr",
